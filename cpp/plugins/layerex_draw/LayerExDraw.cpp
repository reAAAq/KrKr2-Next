 ...
 */
std::pair<std::filesystem::path, std::string> findFontPath(const std::string &familyName) {
    ...
    FcConfigAppFontAddDir(fcConfig, reinterpret_cast<const FcChar8 *>("/system/fonts"));

    pattern = FcPatternCreate();

    FcPatternAddString(pattern, FC_FAMILY, reinterpret_cast<const u_char *>(familyName.c_str()));
    ...
    spdlog::get("plugin")->info("match font family is {}", reinterpret_cast<char *>(fontFamily));

    return { reinterpret_cast<char *>(filePath), reinterpret_cast<char *>(fontFamily) };
}
...
void GdiPlus::addPrivateFont(const tjs_char *fontFileName) {
    spdlog::get("plugin")->info("tjs2 script want load: {}", ttstr{ fontFileName }.AsNarrowStdString());
    // HOOK Font File
...
bool loadFontFromAssets(FT_Library library, FT_Face *face) {
    ...
    if(!G_FontFamilyData) {
        ...
        G_FontFamilyData = fileUtilsInst->getFileData("DroidSansFallback.ttf", "r", &G_FontFamilyDataSize);
    ...
    FT_Error code = FT_New_Memory_Face(library, G_FontFamilyData, G_FontFamilyDataSize, 0, face);
...
FontInfo::FontInfo(const FontInfo &orig) {
    ...
    const FT_Open_Args args{ .flags = FT_OPEN_PATHNAME, .pathname = ftFace->family_name };
...
bool FontInfo::getSelfPathDraw() const { return forceSelfPathDraw || gdiPlusUnsupportedFont; }
...
void commonBrushParameter(ncbPropAccessor &info, T *brush) {
    ...
    if(info.checkVariant(TJS_W("blendBellShape"), var)) {
        ...
        if(IsArray(var)) {
            brush->SetBlendBellShape((REAL)sinfo.getRealValue(0), (REAL)sinfo.getRealValue(1));
        } else {
            brush->SetBlendBellShape((REAL)info.getRealValue(TJS_W("focus")), (REAL)info.getRealValue(TJS_W("scale")));
        }
    }
    ...
    if(info.checkVariant(TJS_W("blendTriangularShape"), var)) {
        ncbPropAccessor sinfo(var);
        if(IsArray(var)) {
            brush->SetBlendTriangularShape((REAL)sinfo.getRealValue(0), (REAL)sinfo.getRealValue(1));
        } else {
    ...
    if(colorOrBrush.Type() != tvtObject) {
        ...
        switch(type) {
            case BrushTypeSolidColor:
                brush = new SolidBrush{ Color{ (ARGB)info.getRealValue(TJS_W("color"), 0xffffffff) } };
                break;
            ...
            case BrushTypePathGradient: {
                ...
                if(info.checkVariant(TJS_W("focusScales"), var)) {
                    ncbPropAccessor sinfo(var);
                    if(IsArray(var)) {
                        pbrush->SetFocusScales((REAL)sinfo.getRealValue(0), (REAL)sinfo.getRealValue(1));
                    } else {
            ...
            case BrushTypeLinearGradient: {
                ...
                if(info.checkVariant(TJS_W("point1"), var)) {
                    ...
                    if(info.HasValue(TJS_W("angle"))) {
                        // アングル指定がある場合
                        lbrush =
                            new LinearGradientBrush{ rect, color1, color2, (REAL)info.getRealValue(TJS_W("angle"), 0),
                                                     static_cast<bool>(info.getIntValue(TJS_W("isAngleScalable"), 0)) };
                    } else {
                        // 無い場合はモードを参照
...
LayerExDraw::LayerExDraw(DispatchT obj) :
    layerExBase(obj), width(-1), height(-1), pitch(0), bitmap(nullptr), graphics(nullptr), clipLeft(-1), clipTop(-1),
    clipWidth(-1), clipHeight(-1), smoothingMode(SmoothingModeAntiAlias), textRenderingHint(TextRenderingHintAntiAlias),
    metafile(nullptr),
...
void LayerExDraw::reset() {
    layerExBase::reset();
    // 変更されている場合はつくりなおし
    if(!(graphics && width == _width && height == _height && pitch == _pitch && buffer == _buffer)) {
        GdipDeleteGraphics(this->graphics);
        ...
        pitch = _pitch;
        buffer = _buffer;
        GdipCreateBitmapFromScan0(width, height, pitch, PixelFormat32bppARGB, (unsigned char *)buffer, &bitmap);
    ...
    if(_clipLeft != clipLeft || _clipTop != clipTop || _clipWidth != clipWidth || _clipHeight != clipHeight) {
        clipLeft = _clipLeft;
        clipTop = _clipTop;
            ...
            switch(i->type) {
                case 0: {
                    ...
                    if(first) {
                        ...
                        RectFClass r{};
                        GdipGetPathWorldBounds(path, &r, static_cast<GpMatrix *>(matrix), static_cast<GpPen *>(*pen));
                        RectFClass::Union(rect, rect, r);
                    }
                } break;
                case 1:
                    if(first) {
                        GdipGetPathWorldBounds(path, &rect, static_cast<GpMatrix *>(matrix), nullptr);
                        first = false;
                    } else {
                        RectFClass r;
...
void LayerExDraw::draw(GpGraphics *graphics, const Pen *pen, const MatrixClass *matrix, const GpPath *path) {
...
void LayerExDraw::fill(GpGraphics *graphics, const BrushBase *brush, const MatrixClass *matrix, const GpPath *path) {
    GraphicsContainer container{};
    GdipBeginContainer2(graphics, &container);
    GdipMultiplyWorldTransform(graphics, static_cast<GpMatrix *>(*matrix), MatrixOrderPrepend);
    GdipSetSmoothingMode(graphics, smoothingMode);
 ...
 * @param path パス
 */
RectFClass LayerExDraw::drawPath(const Appearance *app, const DrawPath *path) { return _drawPath(app, path->path); }
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawArc(const Appearance *app, REAL x, REAL y, REAL width, REAL height, REAL startAngle,
                                REAL sweepAngle) {
    GpPath *path{};
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawBezier(const Appearance *app, REAL x1, REAL y1, REAL x2, REAL y2, REAL x3, REAL y3, REAL x4,
                                   REAL y4) {
    GpPath *path{};
    GdipCreatePath(FillModeAlternate, &path);
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawClosedCurve2(const Appearance *app, tTJSVariant points, REAL tension) {
    std::vector<PointFClass> ps;
    getPoints(points, ps);
    GpPath *path{};
    GdipCreatePath(FillModeAlternate, &path);
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawCurve2(const Appearance *app, tTJSVariant points, REAL tension) {
    std::vector<PointFClass> ps;
    getPoints(points, ps);
    GpPath *path{};
    GdipCreatePath(FillModeAlternate, &path);
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawCurve3(const Appearance *app, tTJSVariant points, int offset, int numberOfSegments,
                                   REAL tension) {
    std::vector<PointFClass> ps;
    getPoints(points, ps);
    GpPath *path{};
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawPie(const Appearance *app, REAL x, REAL y, REAL width, REAL height, REAL startAngle,
                                REAL sweepAngle) {
    GpPath *path{};
    GdipCreatePath(FillModeAlternate, &path);
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawEllipse(const Appearance *app, REAL x, REAL y, REAL width, REAL height) {
    GpPath *path{};
    GdipCreatePath(FillModeAlternate, &path);
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawLine(const Appearance *app, REAL x1, REAL y1, REAL x2, REAL y2) {
    GpPath *path{};
    GdipCreatePath(FillModeAlternate, &path);
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawRectangle(const Appearance *app, REAL x, REAL y, REAL width, REAL height) {
    GpPath *path{};
    GdipCreatePath(FillModeAlternate, &path);
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawPathString(const FontInfo *font, const Appearance *app, REAL x, REAL y,
                                       const tjs_char *text) {
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawString(const FontInfo *font, const Appearance *app, REAL x, REAL y, const tjs_char *text) {
    if(font->getSelfPathDraw())
        return drawPathString2(font, app, x, y, text);
...
RectFClass LayerExDraw::measureStringInternal(const FontInfo *font, const tjs_char *text) {
    ...
    spdlog::get("plugin")->error("gdi+ measure string internal current no implement");
    return {};
    //    const auto *n = reinterpret_cast<const WCHAR *>(text);
    //
    //    GdipSetTextRenderingHint(this->graphics, textRenderingHint);
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawImageRect(REAL dleft, REAL dtop, ImageClass *src, REAL sleft, REAL stop, REAL swidth,
                                      REAL sheight) {
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawImageStretch(REAL dleft, REAL dtop, REAL dwidth, REAL dheight, ImageClass *src, REAL sleft,
                                         REAL stop, REAL swidth, REAL sheight) {
    return drawImageAffine(src, sleft, stop, swidth, sheight, true, dwidth / swidth, 0, 0, dheight / sheight, dleft,
                           dtop);
}
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawImageAffine(ImageClass *src, REAL sleft, REAL stop, REAL swidth, REAL sheight, bool affine,
...
#define AFFINEY(x, y) (B * x + D * y + F)
        ...
        GdipDrawImagePointsRect(this->graphics, static_cast<GpImage *>(*src), points, 3, sleft, stop, swidth, sheight,
...
bool LayerExDraw::redraw(ImageClass *image) {
    if(image) {
        ...
        GdipDrawImageRect(this->graphics, static_cast<GpImage *>(*image), bounds->X, bounds->Y, bounds->Width,
                          bounds->Height);
        GdipSetWorldTransform(this->graphics, static_cast<GpMatrix *>(calcTransform));
        delete bounds;
        _pUpdate(0, nullptr);
 ...
 * @param glyph 描画するグリフ
 */
void LayerExDraw::getGlyphOutline(const FontInfo *fontInfo, PointFClass &offset, GpPath *path, UINT charcode) {
    // 加载字形
    FT_UInt glyphIndex = FT_Get_Char_Index(fontInfo->ftFace, charcode);
    if(glyphIndex == 0) {
        // 不支持此字符
        spdlog::get("plugin")->error("not find Unicode >> {} << in FontFamily",
    ...
    const float scaleFactor = (emSize / unitsPerEM) / adjustmentFactor + 0.009f; // 0.009f修正因子, 弥补精度丢失
    PointFClass glyphOffset{ offset.X,
                             offset.Y + fontInfo->getAscent() + fabs(fontInfo->getDescent()) +
                                 fabs(fontInfo->getDescentLeading()) + fontInfo->getAscentLeading() };

    FT_Outline outline = glyph->outline;

    const auto getPoint = [&](int index) -> PointFClass {
        return { static_cast<float>(outline.points[index].x) * scaleFactor + glyphOffset.X,
                 static_cast<float>(-outline.points[index].y) * scaleFactor + glyphOffset.Y };
    };

    std::function<PointFClass(int, int, int)> getConicEndPoint = [&](int contourStart, int index,
    ...
    const auto addPathConicBezier = [&path](PointFClass startP, PointFClass controlP, PointFClass endP) {
        float t = 2.0f / 3.0f;
        PointFClass cubicP1 = startP + (controlP - startP) * t;
        PointFClass cubicP2 = endP + (controlP - endP) * t;

        GdipAddPathBezier(path, startP.X, startP.Y, cubicP1.X, cubicP1.Y, cubicP2.X, cubicP2.Y, endP.X, endP.Y);
    };
    ...
    for(int i = 0; i < outline.n_contours; ++i) {
        ...
        for(int j = contourStart + 1; j <= contourEnd; ++j) {
            ...
            if(prevTag == FT_CURVE_TAG_CUBIC && currentTag == FT_CURVE_TAG_ON) {
                ...
                g_assert(FT_CURVE_TAG(outline.tags[j - 2]) == FT_CURVE_TAG_CUBIC);

                PointFClass startP = getPoint(j - 3);
                PointFClass control = getPoint(j - 2);
                GdipAddPathBezier(path, startP.X, startP.Y, control.X, control.Y, prevP.X, prevP.Y, currentP.X,
                                  currentP.Y);
                continue;
            }
            ...
            if(prevTag == FT_CURVE_TAG_ON && currentTag == FT_CURVE_TAG_CONIC) {
                addPathConicBezier(prevP, currentP, getConicEndPoint(contourStart, j, contourEnd));
                continue;
            }

            if(prevTag == FT_CURVE_TAG_CONIC && currentTag == FT_CURVE_TAG_CONIC) {
                addPathConicBezier((prevP + currentP) / 2.0f, currentP, getConicEndPoint(contourStart, j, contourEnd));
                continue;
            }
        }
        ...
        if(currentTag == FT_CURVE_TAG_ON && contourStartTag == FT_CURVE_TAG_ON) {
            GdipAddPathLine(path, currentP.X, currentP.Y, contourStartP.X, contourStartP.Y);
        }

        if(currentTag == FT_CURVE_TAG_ON && contourStartTag == FT_CURVE_TAG_CONIC) {
            addPathConicBezier(currentP, contourStartP, getConicEndPoint(contourStart, contourStart, contourEnd));
        }

        if(currentTag == FT_CURVE_TAG_CONIC && contourStartTag == FT_CURVE_TAG_ON) {
            addPathConicBezier(prevTag == FT_CURVE_TAG_ON ? prevP : (prevP + currentP) / 2.0f, currentP, contourStartP);
        }

        if(currentTag == FT_CURVE_TAG_CUBIC) {

            PointFClass startP = getPoint(contourEnd - 2);
            GdipAddPathBezier(path, startP.X, startP.Y, prevP.X, prevP.Y, currentP.X, currentP.Y, contourStartP.X,
                              contourStartP.Y);
        }
 ...
 * @param text 描画するテキスト
 */
void LayerExDraw::getTextOutline(const FontInfo *fontInfo, PointFClass &offset, GpPath *path, const ttstr &text) {
    if(text.IsEmpty())
        return;
 ...
 * @return 更新領域情報
 */
RectFClass LayerExDraw::drawPathString2(const FontInfo *font, const Appearance *app, REAL x, REAL y,
                                        const tjs_char *text) {
    // 文字列のパスを準備
    GpPath *path{};
 ...
 * 画像の保存
 */
tjs_error LayerExDraw::saveImage(tTJSVariant *result, tjs_int numparams, tTJSVariant **param, iTJSDispatch2 *objthis) {
    ...
    if(bitmap) {
        ...
        if(count > 0) {
            ...
            for(int i = 0; i < count; i++) {
                ...
                rarray->FuncCall(0, TJS_W("push"), &pushHint, nullptr, 4, points, rarray);
                tTJSVariant var(rarray, rarray), *param = &var;
                rarray->Release();
                array->FuncCall(0, TJS_W("push"), &pushHint, nullptr, 1, &param, array);
            }
            delete[] rects;
        }
