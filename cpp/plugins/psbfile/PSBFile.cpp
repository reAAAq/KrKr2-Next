...
namespace PSB {
    ...
    bool PSBFile::loadPSBFile(const ttstr &filePath) {
        ...
        if(auto root = getObjects()) {
            std::string rootKeys;
            for(const auto &[k, v] : *root) {
                if(!rootKeys.empty())
                    rootKeys += ", ";
                rootKeys += k + "(" + v->toString() + ")";
            }
            LOGGER->info("PSB loaded: version={}, names={}, strings={}, "
                         "resources={}, rootKeys=[{}]",
                         _header.version, names.size(), strings.size(),
                         resources.size(), rootKeys);
            ...
            for(size_t si = 0; si < strings.size(); si++) {
                if(!allStrings.empty())
                    allStrings += ", ";
                allStrings +=
                    fmt::format("[{}]\"{}\"", si, strings[si].value);
            }
            LOGGER->info("PSB strings: {}", allStrings);

            if(root->find("layers") != root->end()) {
                auto layersVal = (*root)["layers"];
                if(auto *layers = dynamic_cast<PSBList *>(layersVal.get())) {
                    LOGGER->info("PIMG layers count: {}", layers->size());
                    for(size_t i = 0; i < layers->size(); i++) {
                        auto item = (*layers)[static_cast<int>(i)];
                        if(auto *dic =
                               dynamic_cast<PSBDictionary *>(item.get())) {
                            std::string name = "?";
                            std::string nameHex;
                            int layerId = -1;
                            int groupId = -1;
                            int layerType = -1;
                            if(auto n = dynamic_cast<PSBString *>(
                                   (*dic)["name"].get())) {
                                name = n->value;
                            ...
                            if(auto n = dynamic_cast<PSBNumber *>(
                                   (*dic)["layer_id"].get()))
                                layerId = static_cast<int>(*n);
                            if(auto n = dynamic_cast<PSBNumber *>(
                                   (*dic)["group_layer_id"].get()))
                                groupId = static_cast<int>(*n);
                            if(auto n = dynamic_cast<PSBNumber *>(
                                   (*dic)["layer_type"].get()))
                                layerType = static_cast<int>(*n);

                            std::string allKeys;
                            for(const auto &[dk, dv] : *dic) {
                                if(!allKeys.empty())
                                    allKeys += ", ";
                                allKeys += dk;
                            }
                            LOGGER->info(
                                "  layer[{}]: name=\"{}\" hex=[{}] id={} "
                                "group={} type={} keys=[{}]",
                                i, name, nameHex, layerId, groupId,
                                layerType, allKeys);
                        }
                    }
                }
