...
TVPBaseFileSelectorForm::~TVPBaseFileSelectorForm() { CC_SAFE_RELEASE_NULL(CellTemplateForSize); }
...
void TVPBaseFileSelectorForm::bindBodyController(const NodeMap &allNodes) {
    ...
    if(NaviBar.Left) {
        NaviBar.Left->addClickEventListener(
            std::bind(&TVPBaseFileSelectorForm::onBackClicked, this, std::placeholders::_1));
    }
}
...
void TVPBaseFileSelectorForm::ListDir(std::string path) {
    ...
    TVPListDir(path, [&](const std::string &name, int mask) {
        if(mask & (S_IFREG | S_IFDIR)) {
            ...
            std::transform(info.NameForCompare.begin(), info.NameForCompare.end(), info.NameForCompare.begin(),
                           [](int c) -> int {
                               if(c <= 'Z' && c >= 'A')
                                   return c - ('A' - 'a');
                               return c;
...
void TVPBaseFileSelectorForm::onCellLongPress(int idx) {
        ...
        if(!_fileOperateMenu) {
            ...
            if((btn = reader.findWidget("btnUnselect"))) {
                btn->addClickEventListener(
                    std::bind(&TVPBaseFileSelectorForm::onUnselectClicked, this, std::placeholders::_1));
            }
            if((btn = reader.findWidget("btnView", false))) {
                btn->addClickEventListener(
                    std::bind(&TVPBaseFileSelectorForm::onViewClicked, this, std::placeholders::_1));
            }
            if((btn = reader.findWidget("btnCopy"))) {
                btn->addClickEventListener(
                    std::bind(&TVPBaseFileSelectorForm::onCopyClicked, this, std::placeholders::_1));
            }
            if((btn = reader.findWidget("btnCut"))) {
                btn->addClickEventListener(
                    std::bind(&TVPBaseFileSelectorForm::onCutClicked, this, std::placeholders::_1));
            }
            if((btn = reader.findWidget("btnPaste"))) {
                btn->addClickEventListener(
                    std::bind(&TVPBaseFileSelectorForm::onPasteClicked, this, std::placeholders::_1));
            }
            if((btn = reader.findWidget("btnUnpack", false))) {
                btn->addClickEventListener(
                    std::bind(&TVPBaseFileSelectorForm::onUnpackClicked, this, std::placeholders::_1));
            }
            if((btn = reader.findWidget("btnDelete"))) {
                btn->addClickEventListener(
                    std::bind(&TVPBaseFileSelectorForm::onDeleteClicked, this, std::placeholders::_1));
            }
            if((btn = reader.findWidget("btnSendTo"))) {
                btn->addClickEventListener(
                    std::bind(&TVPBaseFileSelectorForm::onSendToClicked, this, std::placeholders::_1));
            }
            if((btn = reader.findWidget("btnRename"))) {
                btn->addClickEventListener(
                    std::bind(&TVPBaseFileSelectorForm::onBtnRenameClicked, this, std::placeholders::_1));
            }
    ...
    int n = TVPShowSimpleMessageBox(localeMgr->GetText("file_operate_menu_text").c_str(),
...
void TVPBaseFileSelectorForm::onTitleClicked(cocos2d::Ref *owner) {
    ...
    for(const std::string &path : paths) {
        CSBReader reader;
        auto *cell = dynamic_cast<Widget *>(reader.Load("ui/ListItem.csb")->getChildByName("item"));
...
void TVPBaseFileSelectorForm::onBackClicked(cocos2d::Ref *owner) { ListDir(ParentPath); }

TVPBaseFileSelectorForm::FileItemCellImpl *
TVPBaseFileSelectorForm::FetchCell(FileItemCellImpl *CellModel, cocos2d::extension::TableView *table, ssize_t idx) {
    if(!CellModel) {
...
void TVPBaseFileSelectorForm::onUnselectClicked(cocos2d::Ref *owner) { clearFileMenu(); }
    ...
    public:
        bool Init(const std::string &path, const std::string &outpath) {
            ...
            ArcUnpacker.SetCallback(
                ...
                std::bind(&UnpackArchive::OnNewFile, this, std::placeholders::_1, std::placeholders::_2,
                          std::placeholders::_3),
            ...
            if(TotalSize > 1024 * 1024) {
                ...
                vecButtons.emplace_back("Stop", [this](Ref *) { ArcUnpacker.Stop(); });
...
void TVPBaseFileSelectorForm::onDeleteClicked(cocos2d::Ref *owner) {
    ...
    if(TVPShowSimpleMessageBoxYesNo(content, localeMgr->GetText("ensure_to_delete_file")) == 0) {
        ...
        scheduleOnce([this](float) { ListDir(CurrentPath); }, 0, "refresh_path");
    }
}
...
void TVPBaseFileSelectorForm::updateFileMenu() {
    ...
    if(_selectedFileIndex.size() == 1) {
        if(_fileOperateCell_view)
            _fileOperateMenulist->pushBackCustomItem(_fileOperateCell_view.get());
...
void TVPListForm::show() {
    TVPMainScene::setMaskLayTouchBegain(
        std::bind(&TVPListForm::onMaskTouchBegan, this, std::placeholders::_1, std::placeholders::_2));
...
std::string TVPShowFileSelector(const std::string &title, const std::string &initfilename, std::string initdir,
                                bool issave) {
    if(initdir.empty()) {
...
TVPFileSelectorForm *TVPFileSelectorForm::create(const std::string &initfilename, const std::string &initdir,
                                                 bool issave) {
