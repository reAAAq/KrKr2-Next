...
static void PreferenceSetValueBool(const std::string &name, bool v) { GetConfigManager()->SetValueInt(name, v); }
...
static void PreferenceSetValueFloat(const std::string &name, float v) { GetConfigManager()->SetValueFloat(name, v); }
...
namespace {
    ...
    public:
        ...
        virtual iPreferenceItem *createItem(int idx) override {
            ...
            return CreatePreferenceItem<tPreferenceItemCheckBox>(
                idx, PrefListSize, locmgr->GetText(Caption), [this](tPreferenceItemCheckBox *item) {
    ...
    public:
        ...
        virtual iPreferenceItem *createItem(int idx) override {
            ...
            return CreatePreferenceItem<tPreferenceItemSelectList>(
                idx, PrefListSize, locmgr->GetText(Caption), [this](tPreferenceItemSelectList *item) {
                    item->initInfo(this);
                    item->_getter = std::bind(&PreferenceGetValueString, Key, DefaultValue);
                    item->_setter = [this](std::string v) { onSetValue(v); };
                });
        }
        virtual const std::vector<std::pair<std::string, std::string>> &getListInfo() override { return ListInfo; }
        virtual void onSetValue(const std::string &v) { PreferenceSetValueString(Key, v); }
    ...
    public:
        tTVPPreferenceInfoTextureCompressionSelectList(
            const std::string &cap, const std::string &key, const std::string &defval,
            const std::initializer_list<std::tuple<std::string, std::string, unsigned int>> &listinfo) :
            tTVPPreferenceInfoSelectList(cap, key, defval,
    ...
    public:
        ...
        virtual iPreferenceItem *createItem(int idx) override {
            ...
            return CreatePreferenceItem<tPreferenceItemFileSelect>(
                idx, PrefListSize, locmgr->GetText(Caption), [this](tPreferenceItemFileSelect *item) {
                    item->_getter = std::bind(&PreferenceGetValueString, Key, DefaultValue);
    ...
    public:
        ...
        virtual iPreferenceItem *createItem(int idx) override {
            LocaleConfigManager *locmgr = LocaleConfigManager::GetInstance();
            iPreferenceItem *ret =
                CreatePreferenceItem<tPreferenceItemSubDir>(idx, PrefListSize, locmgr->GetText(Caption));
    ...
    public:
        ...
        virtual iPreferenceItem *createItem(int idx) override {
            ...
            tPreferenceItemCursorSlider *ret =
                new tPreferenceItemCursorSlider(DefaultValue, TVPMainScene::convertCursorScale);
            ret->autorelease();
    ...
    public:
        ...
        virtual iPreferenceItem *createItem(int idx) override {
            ...
            tPreferenceItemConstant *ret =
                CreatePreferenceItem<tPreferenceItemConstant>(idx, PrefListSize, locmgr->GetText(Caption));
            ret->setTouchEnabled(true);
            ret->addClickEventListener([](Ref *) {
                LocaleConfigManager *locmgr = LocaleConfigManager::GetInstance();
                if(TVPShowSimpleMessageBoxYesNo(locmgr->GetText("preference_ensure_reset_skin"),
    ...
    static void initAllConfig() {
        ...
        RootPreference.Preferences = {
            ...
            new tTVPPreferenceInfoSelectList(
                ...
                { { "preference_opengl", "opengl" }, { "preference_software", "software" } }),
...
#ifdef CC_TARGET_OS_IPHONE
                    new tTVPPreferenceInfoCheckBox("GL_EXT_shader_framebuffer_fetch", "GL_EXT_shader_framebuffer_fetch",
                                                   true),
#else
                    new tTVPPreferenceInfoCheckBox("GL_EXT_shader_framebuffer_fetch", "GL_EXT_shader_framebuffer_fetch",
                                                   false),
#endif
                    new tTVPPreferenceInfoCheckBox("GL_ARM_shader_framebuffer_fetch", "GL_ARM_shader_framebuffer_fetch",
                                                   true),
                    new tTVPPreferenceInfoCheckBox("GL_NV_shader_framebuffer_fetch", "GL_NV_shader_framebuffer_fetch",
                                                   true),
