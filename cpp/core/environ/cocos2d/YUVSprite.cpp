...
static const char *_yuvRenderProgram_fsh = "#ifdef GL_ES"
                                           "\n"
...
static cocos2d::GLProgram *_getYUVRenderProgram() {
    ...
    if(!_program) {
        ...
        if(!_backgroundListener) {
            _backgroundListener =
                cocos2d::EventListenerCustom::create(EVENT_RENDERER_RECREATED, [](cocos2d::EventCustom *) {
                    _program->reset();
...
void TVPYUVSprite::setupVBOAndVAO() {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(_polyInfo.triangles.verts[0]) * _polyInfo.triangles.vertCount,
    ...
    glVertexAttribPointer(cocos2d::GLProgram::VERTEX_ATTRIB_POSITION, 3, GL_FLOAT, GL_FALSE,
                          sizeof(_polyInfo.triangles.verts[0]), (GLvoid *)offsetof(cocos2d::V3F_C4B_T2F, vertices));
    ...
    glVertexAttribPointer(cocos2d::GLProgram::VERTEX_ATTRIB_COLOR, 4, GL_UNSIGNED_BYTE, GL_TRUE,
                          sizeof(_polyInfo.triangles.verts[0]), (GLvoid *)offsetof(cocos2d::V3F_C4B_T2F, colors));
    ...
    glVertexAttribPointer(cocos2d::GLProgram::VERTEX_ATTRIB_TEX_COORD, 2, GL_FLOAT, GL_FALSE,
                          sizeof(_polyInfo.triangles.verts[0]), (GLvoid *)offsetof(cocos2d::V3F_C4B_T2F, texCoords));

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _buffersVBO[1]);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(_polyInfo.triangles.indices[0]) * _polyInfo.triangles.indexCount,
...
void TVPYUVSprite::updateTextureDataInternal(cocos2d::Texture2D *pTex, const void *data, int width, int height,
    ...
    if(size.width != videoSize.width || size.height != videoSize.height || pTex->getPixelFormat() != pixfmt) {
        if(size.width < videoSize.width || size.height < videoSize.height || pTex->getPixelFormat() != pixfmt) {
            ...
            pTex->initWithData(data, datasize, pixfmt, width, height, cocos2d::Size::ZERO);
            pTex = nullptr;
        }
    }
    if(pTex) {
...
void TVPYUVSprite::updateTextureData(const void *Y, int YW, int YH, const void *U, int UW, int UH, const void *V,
                                     int VW, int VH) {
    cocos2d::Texture2D *pTex = getTexture();
    const cocos2d::Size &size = pTex->getContentSize();
...
void TVPYUVSprite::onDraw() {
    ...
    cocos2d::Vec3 vertices[4] = { _polyInfo.triangles.verts[0].vertices, _polyInfo.triangles.verts[1].vertices,
                                  _polyInfo.triangles.verts[2].vertices, _polyInfo.triangles.verts[3].vertices };
    glVertexAttribPointer(cocos2d::GLProgram::VERTEX_ATTRIB_POSITION, 3, GL_FLOAT, GL_FALSE, 0, vertices);
    cocos2d::Color4B colors[4] = { _polyInfo.triangles.verts[0].colors, _polyInfo.triangles.verts[1].colors,
                                   _polyInfo.triangles.verts[2].colors, _polyInfo.triangles.verts[3].colors };
    glVertexAttribPointer(cocos2d::GLProgram::VERTEX_ATTRIB_COLOR, 4, GL_UNSIGNED_BYTE, GL_TRUE, 0, colors);
    cocos2d::Tex2F texCoords[4] = { _polyInfo.triangles.verts[0].texCoords, _polyInfo.triangles.verts[1].texCoords,
                                    _polyInfo.triangles.verts[2].texCoords, _polyInfo.triangles.verts[3].texCoords };
