    ...
    if(c < 0x80) {
        ...
        *pwc = ((ucs4_t)(c & 0x07) << 18) | ((ucs4_t)(s[1] ^ 0x80) << 12) | ((ucs4_t)(s[2] ^ 0x80) << 6) |
        ...
        if(!((s[1] ^ 0x80) < 0x40 && (s[2] ^ 0x80) < 0x40 && (s[3] ^ 0x80) < 0x40 && (s[4] ^ 0x80) < 0x40 &&
             (c >= 0xf9 || s[1] >= 0x88)))
            return RET_ILSEQ;
        *pwc = ((ucs4_t)(c & 0x03) << 24) | ((ucs4_t)(s[1] ^ 0x80) << 18) | ((ucs4_t)(s[2] ^ 0x80) << 12) |
            ((ucs4_t)(s[3] ^ 0x80) << 6) | (ucs4_t)(s[4] ^ 0x80);
        return 5;
        ...
        if(!((s[1] ^ 0x80) < 0x40 && (s[2] ^ 0x80) < 0x40 && (s[3] ^ 0x80) < 0x40 && (s[4] ^ 0x80) < 0x40 &&
            ...
            return RET_ILSEQ;
        *pwc = ((ucs4_t)(c & 0x01) << 30) | ((ucs4_t)(s[1] ^ 0x80) << 24) | ((ucs4_t)(s[2] ^ 0x80) << 18) |
            ((ucs4_t)(s[3] ^ 0x80) << 12) | ((ucs4_t)(s[4] ^ 0x80) << 6) | (ucs4_t)(s[5] ^ 0x80);
        return 6;
    } else
        return RET_ILSEQ;
}

static int utf8_wctomb(/*conv_t conv,*/ unsigned char *r, ucs4_t wc, int n) /* n == 0 is acceptable */
{
    int count;
    if(wc < 0x80)
        count = 1;
