...
std::string checkTextEncoding(const void* buf, size_t size, std::uint8_t &bomSize) {
    ...
    if (size >= 2 && raw[0] == 0xFF && raw[1] == 0xFE) {
        ...
        bomSize = 3;
        encoding = "UTF-8";
    } else if (size >= 4 && raw[0] == 0xFF && raw[1] == 0xFE && raw[2] == 0x00 && raw[3] == 0x00) {
        // UTF-32LE BOM
        bomSize = 4;
        encoding = "UTF-32LE";
    } else if (size >= 4 && raw[0] == 0x00 && raw[1] == 0x00 && raw[2] == 0xFE && raw[3] == 0xFF) {
        // UTF-32BE BOM
        bomSize = 4;
        encoding = "UTF-32BE";
    } else {
...
public:
    tTVPTextReadStream(const ttstr &name, const ttstr &mode) {
        ...
        if (size >= 3 && raw[0] == 0xFE && raw[1] == 0xFE) {
            ...
            if (m == 0 || m == 1) {
                // 解密 UTF-16 数据
                const auto* src = reinterpret_cast<const char16_t*>(raw.data() + 4);
                size_t len = (size - 4) / 2;
                _buffer.resize(len);
        ...
        if(encoding == "UTF-16" || encoding == "UTF-16LE" || encoding == "UTF-16BE") {
            _buffer.assign(reinterpret_cast<const char16_t*>(raw.data()),
        ...
        try {
            std::wstring wide = boost::locale::conv::to_utf<wchar_t>(
                reinterpret_cast<const char *>(raw.data()),
                reinterpret_cast<const char *>(raw.data() + raw.size()), encoding);
    ...
    tjs_uint Read(tTJSString &targ, tjs_uint size) override {
        static_assert(sizeof(tjs_char) == sizeof(char16_t), "Char size mismatch");
...
class tTVPTextWriteStream : public iTJSTextWriteStream {
    ...
    std::vector<Bytef> _compressionBuffer = std::vector<Bytef>(COMPRESSION_BUFFER_SIZE);
...
public:
    tTVPTextWriteStream(const ttstr &name, const ttstr &mode) {
        ...
        // check c/z mode
        _cryptMode = parseModeNumber(mode.c_str(), TJS_W('c'), 1, -1).value_or(1);

        if(auto z = parseModeNumber(mode.c_str(), TJS_W('z'), 1, Z_DEFAULT_COMPRESSION)) {
