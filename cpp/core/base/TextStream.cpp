    ...
    if(c < 0x80) {
        ...
        *pwc = ((unsigned short)(c & 0x1f) << 6) | (unsigned short)(s[1] ^ 0x80);
        ...
        if(!((s[1] ^ 0x80) < 0x40 && (s[2] ^ 0x80) < 0x40 && (c >= 0xe1 || s[1] >= 0xa0)))
            return -1;
        *pwc =
            ((unsigned short)(c & 0x0f) << 12) | ((unsigned short)(s[1] ^ 0x80) << 6) | (unsigned short)(s[2] ^ 0x80);
        return 3;
    } else
        return -1;
}
...
static size_t _TextStream_mbstowcs(int (*func_mbtowc)(unsigned short *, const unsigned char *), tjs_char *pwcs,
                                   const tjs_nchar *s, size_t n) {
    if(!s)
        return -1;
    if(pwcs && n == 0)
        return 0;
            ...
            if(mark[0] == 0xff && mark[1] == 0xfe) {
                ...
                if(CryptMode == 2) {
                    ...
                    try {
                        ...
                        Buffer = new tjs_char[(BufferLen = (destlen = (unsigned long)uncompressed) / 2) + 1];
                        int result = uncompress(/* uncompress from zlib */
                                                (unsigned char *)Buffer, &destlen, (unsigned char *)nbuf,
                ...
                if(mark[0] == 0xef && mark[1] == 0xbb && mark[2] == 0xbf) {
                    ...
                    try {
                        ...
                        BufferLen = TVPUtf8ToWideCharString((const char *)nbuf, nullptr);
                    ...
                    try {
                        Stream->ReadBuffer(nbuf, size);
                        nbuf[size] = 0; // terminater
                        BufferLen = TextStream_mbstowcs(nullptr, (tjs_nchar *)nbuf, 0);
                        if(BufferLen == (size_t)-1) {
...
public:
    tTVPTextWriteStream(const ttstr &name, const ttstr &modestr) {
        ...
        if(CryptMode != -1 && CryptMode != 1 && CryptMode != 2)
            TVPThrowExceptionMessage(TVPUnsupportedModeString, TJS_W("unsupported cipher mode"));
...
//---------------------------------------------------------------------------
iTJSTextReadStream *TVPCreateTextStreamForReadByEncoding(const ttstr &name, const ttstr &modestr) {
    return new tTVPTextReadStream(name, modestr);
}
...
//---------------------------------------------------------------------------
const tjs_char *TVPGetDefaultReadEncoding() { return DefaultReadEncoding.c_str(); }
//---------------------------------------------------------------------------
