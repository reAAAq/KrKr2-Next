...
void tTVPWaveSegmentQueue::Enqueue(const tTVPWaveSegment &segment) {
    if(Segments.size() > 0) {
        ...
        if(last.Start + last.Length == segment.Start &&
           (double)last.FilteredLength / last.Length == (double)segment.FilteredLength / segment.Length) {
...
//---------------------------------------------------------------------------
void tTVPWaveSegmentQueue::Enqueue(const tTVPWaveLabel &Label) { Labels.push_back(Label); }
//---------------------------------------------------------------------------

//---------------------------------------------------------------------------
void tTVPWaveSegmentQueue::Enqueue(const std::deque<tTVPWaveSegment> &segments) {
    // segment の追加
    for(std::deque<tTVPWaveSegment>::const_iterator i = segments.begin(); i != segments.end(); i++)
        Enqueue(*i);
}
//---------------------------------------------------------------------------
...
void tTVPWaveSegmentQueue::Dequeue(tTVPWaveSegmentQueue &dest, tjs_int64 length) {
    ...
    while(Segments.size() > 0 && remain > 0) {
        if(Segments.front().FilteredLength <= remain) {
            ...
            // Length は 線形補間を行う
            tjs_int64 newlength = static_cast<tjs_int64>((double)Segments.front().Length /
...
void tTVPWaveSegmentQueue::Scale(tjs_int64 new_total_filtered_length) {
    ...
    for(std::deque<tTVPWaveSegment>::iterator i = Segments.begin(); i != Segments.end(); i++) {
        tjs_int64 old_end = offset_was + i->FilteredLength;
        offset_was += i->FilteredLength;

        // old_end は全体から見てどの位置にある？
        double ratio = static_cast<double>(old_end) / static_cast<double>(total_length_was);
    ...
    // からっぽのSegments の除去
    for(std::deque<tTVPWaveSegment>::iterator i = Segments.begin(); i != Segments.end();) {
...
tjs_int64 tTVPWaveSegmentQueue::FilteredPositionToDecodePosition(tjs_int64 pos) const {
    ...
    for(std::deque<tTVPWaveSegment>::const_iterator i = Segments.begin(); i != Segments.end(); i++) {
        if(offset_filtered <= pos && pos < offset_filtered + i->FilteredLength) {
            ...
            return (tjs_int64)(i->Start + (pos - offset_filtered) * (double)i->Length / (double)i->FilteredLength);
        }
