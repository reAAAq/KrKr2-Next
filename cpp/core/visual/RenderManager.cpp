...
static void TVPFreeBitmapBits(void *ptr) {
    if(ptr) {
        ...
        if(~(*(tjs_uint32 *)(bptr - sizeof(tjs_uint32))) != record->sentinel_backup1)
            TVPThrowExceptionMessage(TJS_W("Layer bitmap: Buffer underrun detected. Check your "
                                           "drawing code!"));
        if(~(*(tjs_uint32 *)(bptr + record->size)) != record->sentinel_backup2)
            TVPThrowExceptionMessage(TJS_W("Layer bitmap: Buffer overrun detected. Check your "
                                           "drawing code!"));
...
public:
    ...
    tTVPSoftwareTexture2D_static(const void *pixel, int pitch, unsigned int w, unsigned int h,
                                 TVPTextureFormat::e format) :
    ...
    virtual const void *GetScanLineForRead(tjs_uint l) { return BmpData + Pitch * l; }
    ...
    virtual cocos2d::Texture2D *GetAdapterTexture(cocos2d::Texture2D *origTex) override {
        if(!origTex || origTex->getPixelsWide() != Width || origTex->getPixelsHigh() != Height) {
            ...
            origTex->autorelease();
            origTex->initWithData(BmpData, Pitch * Height, CCPixelFormat::RGBA8888, Pitch / 4, Height,
                                  cocos2d::Size::ZERO);
        } else {
...
public:
    ...
    virtual cocos2d::Texture2D *GetAdapterTexture(cocos2d::Texture2D *origTex) override {
        ...
        if(!origTex || origTex->getPixelsWide() != Width || origTex->getPixelsHigh() != Height) {
            origTex = new cocos2d::Texture2D;
            origTex->autorelease();
            origTex->initWithData(BmpData, Pitch * Height, CCPixelFormat::RGBA8888, Width, Height, cocos2d::Size::ZERO);
        } else {
...
public:
    tTVPSoftwareTexture2D_half(tTVPBitmap *bmp, const void *pixel, int pitch, unsigned int w, unsigned int h,
                               TVPTextureFormat::e format) :
    ...
    static iTVPTexture2D *Create(tTVPBitmap *bmp, const void *pixel, int pitch, unsigned int w, unsigned int h,
                                 TVPTextureFormat::e format) {
        return new tTVPSoftwareTexture2D_half(bmp, pixel, pitch, w, h, format);
    }

    virtual const void *GetScanLineForRead(tjs_uint l) override { return _scanline[l / 2]; }
    ...
    virtual cocos2d::Texture2D *GetAdapterTexture(cocos2d::Texture2D *origTex) override {
        if(!origTex || origTex->getPixelsWide() != Width || origTex->getPixelsHigh() != _scanline.size()) {
            origTex = new cocos2d::Texture2D;
            origTex->autorelease();
            origTex->initWithData(nullptr, Pitch * _scanline.size(), CCPixelFormat::RGBA8888, Width, _scanline.size(),
                                  cocos2d::Size::ZERO);
        }
        int y = 0;
    ...
    virtual size_t GetBitmapSize() override {
        return Pitch * _scanlineData.size() * (Format == TVPTextureFormat::RGBA ? 4 : 1);
    }
};
...
public:
    ...
    static iTVPTexture2D *CreateFromBitmap(tTVPBitmap *bmp) { return new tTVPSoftwareTexture2D(bmp); }
...
public:
    virtual void DoRender(iTVPTexture2D *_tar, const tTVPRect &rctar, iTVPTexture2D *_dst, const tTVPRect &rcdst,
                          iTVPTexture2D *_src, const tTVPRect &rcsrc, iTVPTexture2D *rule, const tTVPRect &rcrule) = 0;
...
template <typename TSrc, typename TDst, void (*&FuncWithOpa)(TDst *, const TSrc *, tjs_int, tjs_uint32, tjs_int),
...
public:
    ...
    virtual void DoRender(iTVPTexture2D *_tar, const tTVPRect &rctar, iTVPTexture2D *_dst, const tTVPRect &rcdst,
                          iTVPTexture2D *_src, const tTVPRect &rcsrc, iTVPTexture2D *rule, const tTVPRect &rcrule) {
...
public:
    ...
    virtual void DoRender(iTVPTexture2D *_tar, const tTVPRect &rect, iTVPTexture2D *_dst, const tTVPRect &rcdst,
                          iTVPTexture2D *_src, const tTVPRect &rcsrc, iTVPTexture2D *rule, const tTVPRect &rcrule) {
...
public:
    virtual void DoRender(iTVPTexture2D *_tar, const tTVPRect &rect, iTVPTexture2D *_dst, const tTVPRect &rcdst,
                          iTVPTexture2D *_src, const tTVPRect &rcsrc, iTVPTexture2D *rule,
...
public:
    virtual void DoRender(iTVPTexture2D *_tar, const tTVPRect &rctar, iTVPTexture2D *_dst, const tTVPRect &rcdst,
        ...
        TVPExecThreadTask(taskNum, [=](int i) {
            ...
            y0 = h * i / taskNum;
            y1 = h * (i + 1) / taskNum;
            this->PartialCopy(_tar, rctar.left, rctar.top + y0, _src, rcsrc.left, rcsrc.top + y0, w, y1 - y0,
                              backwardCopy);
            // 			PartialParam _param;
    ...
    virtual void PartialCopy(iTVPTexture2D *dst, tjs_int dx, tjs_int dy, iTVPTexture2D *src, tjs_int sx, tjs_int sy,
...
public:
            ...
            if(pixelsize == 1) { // 8bit
                for(int y = 0; y < h; ++y) {
                    memcpy((tjs_uint8 *)_dst->GetScanLineForWrite(rctar.top + y) + rctar.left,
                           (const tjs_uint8 *)_src->GetScanLineForRead(rcsrc.top - y - 1) + rcsrc.left, wbytes);
                }
            } else { // 32bit
                for(int y = 0; y < h; ++y) {
                    memcpy((tjs_uint32 *)_dst->GetScanLineForWrite(rctar.top + y) + rctar.left,
                           (const tjs_uint32 *)_src->GetScanLineForRead(rcsrc.top - y - 1) + rcsrc.left, wbytes);
                }
            }
    ...
    void PartialCopy(iTVPTexture2D *dst, tjs_int dx, tjs_int dy, iTVPTexture2D *src, tjs_int sx, tjs_int sy, tjs_int w,
...
public:
    ...
    virtual void PartialFill(iTVPTexture2D *dst, iTVPTexture2D *src, tjs_int sx, tjs_int sy, tjs_int dx, tjs_int dy,
                             tjs_int w, tjs_int h) = 0;
};
...
public:
    virtual void DoRender(iTVPTexture2D *_tar, const tTVPRect &rctar, iTVPTexture2D *_dst, const tTVPRect &rcdst,
        ...
        TVPExecThreadTask(taskNum, [&](int i) {
            ...
            y0 = h * i / taskNum;
            y1 = h * (i + 1) / taskNum;
            this->PartialProc(_tar, rctar.left, rctar.top + y0, _src, rcsrc.left, rcsrc.top + y0, _dst, rcdst.left,
    ...
    virtual void PartialProc(iTVPTexture2D *tar, tjs_int tx, tjs_int ty, iTVPTexture2D *src, tjs_int sx, tjs_int sy,
                             iTVPTexture2D *dst, tjs_int dx, tjs_int dy, tjs_int w, tjs_int h) {
        for(tjs_int y = 0; y < h; ++y) {
...
template <int THREAD_FACTOR, void (*&FuncInitTable)(tjs_uint32 *, tjs_int, tjs_int),
          void (*&Func)(tjs_uint32 *, const tjs_uint32 *, const tjs_uint32 *, const tjs_uint8 *, const tjs_uint32 *,
                        tjs_int),
          void (*&FuncSwitch)(tjs_uint32 *, const tjs_uint32 *, const tjs_uint32 *, const tjs_uint8 *,
...
public:
    virtual void DoRender(iTVPTexture2D *_tar, const tTVPRect &rctar, iTVPTexture2D *_dst, const tTVPRect &rcdst,
                          iTVPTexture2D *_src, const tTVPRect &rcsrc, iTVPTexture2D *_rule, const tTVPRect &rcrule) {
        tjs_int h = rcsrc.bottom - rcsrc.top;
        tjs_int w = rcsrc.right - rcsrc.left;
        assert(w == rcdst.get_width() && h == rcdst.get_height());
        assert(w == rctar.get_width() && h == rctar.get_height());
        ...
        TVPExecThreadTask(taskNum, [&](int i) {
            ...
            y1 = h * (i + 1) / taskNum;
            this->PartialProc(_tar, rctar.left, rctar.top + y0, _src, rcsrc.left, rcsrc.top + y0, _dst, rcdst.left,
                              rcdst.top + y0, _rule, rcrule.left, rcrule.top + y0, w, y1 - y0);
        });
    }
    virtual int EnumParameterID(const char *name) {
    ...
    virtual void PartialProc(iTVPTexture2D *tar, tjs_int tx, tjs_int ty, iTVPTexture2D *src, tjs_int sx, tjs_int sy,
                             iTVPTexture2D *dst, tjs_int dx, tjs_int dy, iTVPTexture2D *rule, tjs_int rx, tjs_int ry,
                             tjs_int w, tjs_int h) {
        // 		tjs_uint8 *dst = p->dest, *src = p->src, *tar = p->tar,
        ...
        if(vague >= 512) {
            ...
            for(tjs_int y = 0; y < h; ++y) {
                FuncSwitch(((tjs_uint32 *)tar->GetScanLineForWrite(ty + y)) + tx,
                           ((const tjs_uint32 *)src->GetScanLineForRead(sy + y)) + sx,
                           ((const tjs_uint32 *)dst->GetScanLineForRead(dy + y)) + dx,
                           ((const tjs_uint8 *)rule->GetScanLineForRead(ry + y)) + rx, BlendTable, w, src1lv, src2lv);
            }
            // 			while (height--)
            // 			{
...
public:
    ...
    virtual void SetParameterOpa(int id, int v) { opa = v; }
    virtual void PartialFill(iTVPTexture2D *_dst, iTVPTexture2D *src, tjs_int sx, tjs_int sy, tjs_int dx, tjs_int dy,
                             tjs_int w, tjs_int h) {
        if(opa == 255) {
            for(tjs_int y = 0; y < h; ++y) {
...
void TVPAddSubVertSum(base_int_type *dest, const tjs_uint32 *addline, const tjs_uint32 *subline, tjs_int len) {}
template <>
void TVPAddSubVertSum<tTVPARGB<tjs_uint16>, tjs_uint16>(tjs_uint16 *dest, const tjs_uint32 *addline,
                                                        const tjs_uint32 *subline, tjs_int len) {
    TVPAddSubVertSum16(dest, addline, subline, len);
}
template <>
void TVPAddSubVertSum<tTVPARGB_AA<tjs_uint16>, tjs_uint16>(tjs_uint16 *dest, const tjs_uint32 *addline,
                                                           const tjs_uint32 *subline, tjs_int len) {
    TVPAddSubVertSum16_d(dest, addline, subline, len);
}
template <>
void TVPAddSubVertSum<tTVPARGB<tjs_uint32>, tjs_uint32>(tjs_uint32 *dest, const tjs_uint32 *addline,
                                                        const tjs_uint32 *subline, tjs_int len) {
    TVPAddSubVertSum32(dest, addline, subline, len);
}
template <>
void TVPAddSubVertSum<tTVPARGB_AA<tjs_uint32>, tjs_uint32>(tjs_uint32 *dest, const tjs_uint32 *addline,
                                                           const tjs_uint32 *subline, tjs_int len) {
    TVPAddSubVertSum32_d(dest, addline, subline, len);
}

template <typename tARGB, typename base_int_type>
void TVPDoBoxBlurAvg(tjs_uint32 *dest, base_int_type *sum, const base_int_type *add, const base_int_type *sub,
                     tjs_int n, tjs_int len) {}
template <>
void TVPDoBoxBlurAvg<tTVPARGB<tjs_uint16>, tjs_uint16>(tjs_uint32 *dest, tjs_uint16 *sum, const tjs_uint16 *add,
                                                       const tjs_uint16 *sub, tjs_int n, tjs_int len) {
    TVPDoBoxBlurAvg16(dest, sum, add, sub, n, len);
}
template <>
void TVPDoBoxBlurAvg<tTVPARGB_AA<tjs_uint16>, tjs_uint16>(tjs_uint32 *dest, tjs_uint16 *sum, const tjs_uint16 *add,
                                                          const tjs_uint16 *sub, tjs_int n, tjs_int len) {
    TVPDoBoxBlurAvg16_d(dest, sum, add, sub, n, len);
}
template <>
void TVPDoBoxBlurAvg<tTVPARGB<tjs_uint32>, tjs_uint32>(tjs_uint32 *dest, tjs_uint32 *sum, const tjs_uint32 *add,
                                                       const tjs_uint32 *sub, tjs_int n, tjs_int len) {
    TVPDoBoxBlurAvg32(dest, sum, add, sub, n, len);
}
template <>
void TVPDoBoxBlurAvg<tTVPARGB_AA<tjs_uint32>, tjs_uint32>(tjs_uint32 *dest, tjs_uint32 *sum, const tjs_uint32 *add,
                                                          const tjs_uint32 *sub, tjs_int n, tjs_int len) {
...
static void _log_ptr(const char *n, void *_p, uint l) {
    ...
    TVPConsoleLog("%s %p: %02X %02X %02X %02X %02X %02X %02X %02X ... %p: %02X %02X %02X "
...
struct TDoBoxBlurLoop {
    ...
    static void DoBoxBlurLoop(const tTVPRect &rect, const tTVPRect &area, tjs_int width /*of texture*/,
                              tjs_int height /*of texture*/, tjs_uint32 *line0, tjs_int pitch) {
        ...
        try {
            // allocate buffers
            vert_sum = (tARGB *)TJSAlignedAlloc(sizeof(tARGB) * (vert_sum_right_limit - vert_sum_left_limit + 1 + 1),
            ...
            tjs_int h_init_start = rect.left + area.left; // this always be the same value as
                                                          // vert_sum_left_limit
            if(h_init_start < 0)
                h_init_start = 0;
            ...
            tjs_int right_frac_len = rect.right + area.right >= width ? rect.right + area.right - width + 1 : 0;
            ...
            for(tjs_int y = rect.top; y < rect.bottom; y++) {
                ...
                if(dest_buf_free == 0) {
                    ...
                    memcpy(rect.left + line0 + pitch * (y - dest_buf_size), &dest_buf[dest_buf_wp][0],
                ...
                if(y != rect.bottom - 1) {
                    ...
                    add_line = y + area.bottom + 1 >= height ? nullptr : (line0 + (y + area.bottom + 1) * pitch);
...
public:
    ...
    virtual void DoRender(iTVPTexture2D *tar, const tTVPRect &rctar, iTVPTexture2D *dst, const tTVPRect &rcdst,
                          iTVPTexture2D *src, const tTVPRect &rcsrc, iTVPTexture2D *rule, const tTVPRect &rcrule) {
        // 		tTVPRenderMethod_DirectCopy::DoRender(
        // 			tar, rctar,
        // 			dst, rcdst,
        // 			src, rcsrc,
        ...
        int sw = rcsrc.get_width(), sh = rcsrc.get_height(), dw = rctar.get_width(), dh = rctar.get_height();
...
iTVPRenderMethod *iTVPRenderManager::GetRenderMethod(const char *name, tjs_uint32 *hint) {
    ...
    if(it != AllMethods.end())
        return it->second;
    TVPShowSimpleMessageBox(ttstr(TJS_W("Operation \"")) + name + TJS_W("\" is not supported under ") + GetName() +
                                TJS_W(" mode."),
...
struct tRenderMethodCache {
    struct MethodInfo {
        ...
        void Init(iTVPRenderManager *mgr, const char *nameNormal, const char *nameHDA, const char *nameOpa,
...
void iTVPRenderManager::Initialize() { RenderMethodCache = new tRenderMethodCache(this); }
...
iTVPRenderMethod *iTVPRenderManager::CompileRenderMethod(const char *name, const char *script, int nTex,
                                                         unsigned int flags) {
...
iTVPRenderMethod *iTVPRenderManager::GetOrCompileRenderMethod(const char *name, uint32_t *hint, const char *glsl_script,
...
static iTVPTexture2D *(*_createStaticTexture2D)(tTVPBitmap *bmp, const void *pixel, int pitch, unsigned int w,
...
public:
    void Register_1() {
        ...
        // 		}
        {
            static tTVPRenderMethod_ColorOpacity<tjs_uint8, tjs_uint32, TVPApplyColorMap_HDA_o, TVPApplyColorMap_HDA>
                method;
            RegisterRenderMethod("ApplyColorMap", &method);
        }
        {
        ...
        // --------- UnivTransBlend ----------
        {
            static tTVPRenderMethod_UnivTransBlt<46, TVPInitUnivTransBlendTable_d, TVPUnivTransBlend_d,
                                                 TVPUnivTransBlend_switch_d>
                method;
            RegisterRenderMethod("UnivTransBlend_d", &method);
        }
        {
            static tTVPRenderMethod_UnivTransBlt<46, TVPInitUnivTransBlendTable_a, TVPUnivTransBlend_a,
...
#define REGISER_BLEND_WITH_HDA_4(THREAD_FACTOR, NAME)                                                                  \
    ...
    tTVPSoftwareRenderManager() :
        StretchType(stNearest), tempTexture(nullptr), img_convert_ctx(nullptr), _drawCount(0) {
        _createStaticTexture2D = tTVPSoftwareTexture2D::Create;
        std::string compTexMethod =
            IndividualConfigManager::GetInstance()->GetValue<std::string>("software_compress_tex", "none");
...
#undef REGISER_BLEND_4
    ...
    virtual iTVPTexture2D *CreateTexture2D(tTVPBitmap *bmp) override {
        if(bmp->GetHeight() <= 16) {
            ...
            return _createStaticTexture2D(bmp, bmp->GetBits(), bmp->GetPitch(), bmp->GetWidth(), bmp->GetHeight(),
                                          bmp->GetBPP() == 8 ? TVPTextureFormat::Gray : TVPTextureFormat::RGBA);
        }
    }
    ...
    virtual iTVPTexture2D *CreateTexture2D(tTJSBinaryStream *s) { return nullptr; }
    ...
    void OperateRect(iTVPRenderMethod *method, iTVPTexture2D *tar, tTVPRect rctar, iTVPTexture2D *src, tTVPRect rcsrc) {
        int sw = rcsrc.get_width(), sh = rcsrc.get_height(), dw = rctar.get_width(), dh = rctar.get_height();
...
// #endif
            ...
            img_convert_ctx = sws_getCachedContext(img_convert_ctx, sw, sh, AV_PIX_FMT_RGBA, dw, dh, AV_PIX_FMT_RGBA,
            ...
            ((tTVPRenderMethod_Software *)method)->DoRender(tar, rctar, tar, rctar, tmp, rc, nullptr, rc);
        } else {
            ((tTVPRenderMethod_Software *)method)->DoRender(tar, rctar, tar, rctar, src, rcsrc, nullptr, rcsrc);
        }
    }
    ...
    virtual void OperateRect(iTVPRenderMethod *method, iTVPTexture2D *tar, iTVPTexture2D *reftar, const tTVPRect &rctar,
        ...
        switch(textures.size()) {
            ...
            case 2: // src x dst -> tar
                ((tTVPRenderMethod_Software *)method)
                    ->DoRender(tar, rctar, textures[1].first, textures[1].second, textures[0].first, textures[0].second,
                               nullptr, rctar);
                break;
...
#define TVP_DoBilinearAffineLoop_ARGS sxs, sys, dest, l, len, src, srcpitch, sxl, syl, srccliprect, srcrect
    typedef std::function<void(tjs_int, tjs_int, tjs_uint8 *, tjs_int, tjs_int, const tjs_uint8 *, tjs_int, tjs_int,
                               tjs_int, const tTVPRect &, const tTVPRect &)>
        TAffuncFunc;
...
#define TVP_BILINEAR_FORCE_COND false
            _stretchMethodFactory[GetRenderMethod("Copy")] = [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                if(TVP_BILINEAR_FORCE_COND || StretchType >= stFastLinear) { // bilinear interpolation
                    return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len, const tjs_uint8 *src,
                        ...
                        TVPDoBilinearAffineLoop(tTVPInterpStretchCopyFunctionObject(),
                                                tTVPInterpLinTransCopyFunctionObject(), TVP_DoBilinearAffineLoop_ARGS);
                    };
                    // 				} else { // !hda
                    ...
                    return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len, const tjs_uint8 *src,
                        ...
                        TVPDoAffineLoop(tTVPStretchFunctionObject(TVPStretchCopy),
                                        tTVPAffineFunctionObject(TVPLinTransCopy), TVP_DoAffineLoop_ARGS);
                    };
                }
            };
            _stretchMethodFactory[GetRenderMethod("ConstAlphaBlend")] =
                ...
                if(TVP_BILINEAR_FORCE_COND || StretchType >= stFastLinear) { // bilinear interpolation
                    return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len,
                        ...
                        TVPDoBilinearAffineLoop(tTVPInterpStretchConstAlphaBlendFunctionObject(opa),
                    ...
                    return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len,
                                 const tjs_uint8 *src, tjs_int srcpitch, tjs_int sxl, tjs_int syl,
                                 const tTVPRect &srccliprect, const tTVPRect &srcrect) {
                        TVPDoAffineLoop(tTVPStretchWithOpacityFunctionObject(TVPStretchConstAlphaBlend_HDA, opa),
            ...
            _stretchMethodFactory[GetRenderMethod("CopyOpaqueImage")] =
                ...
                return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len, const tjs_uint8 *src,
                    ...
                    TVPDoAffineLoop(tTVPStretchFunctionObject(TVPStretchCopyOpaqueImage),
                                    tTVPAffineFunctionObject(TVPLinTransCopyOpaqueImage), TVP_DoAffineLoop_ARGS);
                };
            };
            _stretchMethodFactory[GetRenderMethod("ConstAlphaBlend_d")] =
                ...
                return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len, const tjs_uint8 *src,
                             tjs_int srcpitch, tjs_int sxl, tjs_int syl, const tTVPRect &srccliprect,
                             const tTVPRect &srcrect) {
                    TVPDoAffineLoop(tTVPStretchWithOpacityFunctionObject(TVPStretchConstAlphaBlend_d, opa),
            ...
            _stretchMethodFactory[GetRenderMethod("AlphaBlend")] =
                [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                tjs_int opa =
                    static_cast<tTVPRenderMethod_BltAndOpa<52, TVPAlphaBlend_HDA, TVPAlphaBlend_HDA_o> *>(method)->opa;
                if(opa == 255) {
                    return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len, const tjs_uint8 *src,
                              tjs_int srcpitch, tjs_int sxl, tjs_int syl, const tTVPRect &srccliprect,
                              const tTVPRect &srcrect) {
                        TVPDoAffineLoop(tTVPStretchFunctionObject(TVPStretchAlphaBlend_HDA),
                                        tTVPAffineFunctionObject(TVPLinTransAlphaBlend_HDA), TVP_DoAffineLoop_ARGS);
                    };
                } else {
                    return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len,
                                 const tjs_uint8 *src, tjs_int srcpitch, tjs_int sxl, tjs_int syl,
                                 const tTVPRect &srccliprect, const tTVPRect &srcrect) {
                        TVPDoAffineLoop(tTVPStretchWithOpacityFunctionObject(TVPStretchAlphaBlend_HDA_o, opa),
            ...
            _stretchMethodFactory[GetRenderMethod("AlphaBlend_d")] =
                [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                tjs_int opa =
                    static_cast<tTVPRenderMethod_BltAndOpa<52, TVPAlphaBlend_d, TVPAlphaBlend_do> *>(method)->opa;
                if(opa == 255) {
                    return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len, const tjs_uint8 *src,
                              tjs_int srcpitch, tjs_int sxl, tjs_int syl, const tTVPRect &srccliprect,
                              const tTVPRect &srcrect) {
                        TVPDoAffineLoop(tTVPStretchFunctionObject(TVPStretchAlphaBlend_d),
                                        tTVPAffineFunctionObject(TVPLinTransAlphaBlend_d), TVP_DoAffineLoop_ARGS);
                    };
                } else {
                    return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len,
                                 const tjs_uint8 *src, tjs_int srcpitch, tjs_int sxl, tjs_int syl,
                                 const tTVPRect &srccliprect, const tTVPRect &srcrect) {
                        TVPDoAffineLoop(tTVPStretchWithOpacityFunctionObject(TVPStretchAlphaBlend_do, opa),
                ...
                if(opa == 255) {
                    if(TVP_BILINEAR_FORCE_COND || StretchType >= stFastLinear) {
                        return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len,
                                  const tjs_uint8 *src, tjs_int srcpitch, tjs_int sxl, tjs_int syl,
                                  const tTVPRect &srccliprect, const tTVPRect &srcrect) {
                            TVPDoBilinearAffineLoop(tTVPInterpStretchAdditiveAlphaBlendFunctionObject(),
                        ...
                        return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len,
                                  const tjs_uint8 *src, tjs_int srcpitch, tjs_int sxl, tjs_int syl,
                                  const tTVPRect &srccliprect, const tTVPRect &srcrect) {
                            TVPDoAffineLoop(tTVPStretchFunctionObject(TVPStretchAdditiveAlphaBlend_HDA),
                    ...
                    if(TVP_BILINEAR_FORCE_COND || StretchType >= stFastLinear) {
                        return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len,
                                     const tjs_uint8 *src, tjs_int srcpitch, tjs_int sxl, tjs_int syl,
                                     const tTVPRect &srccliprect, const tTVPRect &srcrect) {
                            TVPDoBilinearAffineLoop(tTVPInterpStretchAdditiveAlphaBlend_oFunctionObject(opa),
            ...
            _stretchMethodFactory[GetRenderMethod("AdditiveAlphaBlend_a")] =
                [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                tjs_int opa =
                    static_cast<tTVPRenderMethod_BltAndOpa<52, TVPAdditiveAlphaBlend_a, TVPAdditiveAlphaBlend_ao> *>(
                        method)
                        ->opa;
                if(opa == 255) {
                    return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len, const tjs_uint8 *src,
                    ...
                    return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len,
                                 const tjs_uint8 *src, tjs_int srcpitch, tjs_int sxl, tjs_int syl,
                                 const tTVPRect &srccliprect, const tTVPRect &srcrect) {
                        TVPDoAffineLoop(tTVPStretchWithOpacityFunctionObject(TVPStretchAlphaBlend_ao, opa),
            ...
            _stretchMethodFactory[GetRenderMethod("AlphaBlend_a")] =
                [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                tjs_int opa =
                    static_cast<tTVPRenderMethod_BltAndOpa<52, TVPAlphaBlend_a, TVPAlphaBlend_ao> *>(method)->opa;
                if(opa == 255) {
                    return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len, const tjs_uint8 *src,
                              tjs_int srcpitch, tjs_int sxl, tjs_int syl, const tTVPRect &srccliprect,
                              const tTVPRect &srcrect) {
                        TVPDoAffineLoop(tTVPStretchFunctionObject(TVPStretchAlphaBlend_a),
                                        tTVPAffineFunctionObject(TVPLinTransAlphaBlend_a), TVP_DoAffineLoop_ARGS);
                    };
                } else {
                    return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len,
                                 const tjs_uint8 *src, tjs_int srcpitch, tjs_int sxl, tjs_int syl,
            ...
            _stretchMethodFactory[GetRenderMethod("ConstAlphaBlend_a")] =
                ...
                return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len, const tjs_uint8 *src,
                             tjs_int srcpitch, tjs_int sxl, tjs_int syl, const tTVPRect &srccliprect,
                             const tTVPRect &srcrect) {
                    TVPDoAffineLoop(tTVPStretchWithOpacityFunctionObject(TVPStretchConstAlphaBlend_a, opa),
        ...
        return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l, tjs_int len, const tjs_uint8 *src,
                  tjs_int srcpitch, tjs_int sxl, tjs_int syl, const tTVPRect &srccliprect, const tTVPRect &srcrect) {
            TVPDoBilinearAffineLoop(tTVPInterpStretchCopyFunctionObject(), tTVPInterpLinTransCopyFunctionObject(),
                                    TVP_DoBilinearAffineLoop_ARGS);
        };
    }
    ...
    virtual void OperateTriangles(iTVPRenderMethod *method, int nTriangles, iTVPTexture2D *target,
        ...
        if(nTriangles == 2 && CheckQuad(pttar) && CheckTextureQuad(textures)) {
            bool isSrcRect = isDoubleEqual(srcpt[0].y, srcpt[1].y) && isDoubleEqual(srcpt[1].x, srcpt[5].x) &&
                isDoubleEqual(srcpt[0].x, srcpt[2].x) && isDoubleEqual(srcpt[2].y, srcpt[5].y);
            bool isDstRect = isDoubleEqual(dstpt[0].y, dstpt[1].y) && isDoubleEqual(dstpt[1].x, dstpt[5].x) &&
            ...
            while(isSrcRect && isDstRect) {
                ...
                if(dstrect.left > dstrect.right || dstrect.top > dstrect.bottom || refrect.left > refrect.right ||
            ...
            if(isSrcRect) {
                ...
                if(rcsrc.get_height() > sh)
                    rcsrc.set_height(sh);
                src_img = cv::Mat(rcsrc.get_height(), rcsrc.get_width(), CV_8UC4, (void *)sdata, spitch);
            } else {
            ...
            iTVPTexture2D *tmp = new tTVPSoftwareTexture2D_static(dst_img.ptr(0), dst_img.step1(0), dst_size.width,
            ...
            ((tTVPRenderMethod_Software *)method)->DoRender(target, rcclip, target, rcclip, tmp, rc, nullptr, rc);
            tmp->Release();
        } else {
...
#define TVP_DEFINE_BILINEAR_STRETCH_WITH_OPACITY_FUNCTION(func, one)                                                   \
    ...
    typedef TVP_GL_FUNC_PTR_DECL(void, tTVPAffineFunction,
                                 (tjs_uint32 * dest, tjs_int len, const tjs_uint32 *src, tjs_int sx, tjs_int sy,
    ...
    public:
        ...
        void operator()(tjs_uint32 *dest, tjs_int len, const tjs_uint32 *src, tjs_int sx, tjs_int sy, tjs_int stepx,
...
#define TVP_DEFINE_BILINEAR_AFFINE_WITH_OPACITY_FUNCTION(func, one)                                                    \
    ...
    static void TVPDoBilinearAffineLoop(tFuncStretch stretch, tFuncAffine affine, tjs_int sxs, tjs_int sys,
                                        tjs_uint8 *dest, tjs_int l, tjs_int len, const tjs_uint8 *src, tjs_int srcpitch,
        ...
        while(len_remain > 0) {
            sx = spx >> 16;
            sy = spy >> 16;
            if(sx >= srcrect.left && sx < srcrect.right && sy >= srcrect.top && sy < srcrect.bottom)
                break;
            dp--;
            spx -= sxs;
            spy -= sys;
...
#define FIX_SX_SY                                                                                                      \
    ...
    if(sy >= srccliprect.bottom)                                                                                       \
        ...
        while(len_remain > 0) {
            ...
            affine.DoOnePixel(dp,
                              TVPBlendARGB(TVPBlendARGB(c00, c01, blend_x), TVPBlendARGB(c10, c11, blend_x), blend_y));
        ...
        if(len_remain > 0) {
            ...
            if(sys == 0) {
                ...
                stretch(dp, len_remain, (const tjs_uint32 *)l1, (const tjs_uint32 *)l2, (spy & 0xffff) >> 8, spx, sxs);
            } else {
                // do affine
    ...
    template <typename tFuncStretch, typename tFuncAffine>
    static void TVPDoAffineLoop(tFuncStretch stretch, tFuncAffine affine, tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                                tjs_int l, tjs_int len, const tjs_uint8 *src, tjs_int srcpitch, tjs_int sxl,
                                tjs_int syl, const tTVPRect &srcrect) {
        tjs_int len_remain = len;
    ...
    public:
        ...
        void operator()(tjs_uint32 *dest, tjs_int len, const tjs_uint32 *src, tjs_int srcstart, tjs_int srcstep) {
    ...
    virtual void OperatePerspective(iTVPRenderMethod *method, int nQuads, iTVPTexture2D *target, iTVPTexture2D *reftar,
                                    const tTVPRect &rcclip, const tTVPPointD *pttar /*quad*/,
        ...
        for(int i = 0; i < nQuads; ++i) {
            ...
            if(!processed) {
                ...
                // upper-left, upper-right, bottom-right, bottom-left
                cv::Point2f pts_src[] = { cv::Point2f(srcpt[0].x, srcpt[0].y), cv::Point2f(srcpt[1].x + 1, srcpt[1].y),
                                          cv::Point2f(srcpt[3].x + 1, srcpt[3].y + 1),
                                          cv::Point2f(srcpt[2].x, srcpt[2].y + 1) };
                cv::Point2f pts_dst[] = { cv::Point2f(dstpt[0].x - rcclip.left, dstpt[0].y - rcclip.top),
                                          cv::Point2f(dstpt[1].x - rcclip.left, dstpt[1].y - rcclip.top),
...
void TVPRegisterRenderManager(const char *name, iTVPRenderManager *(*func)()) {
    if(!_RenderManagerFactory)
        _RenderManagerFactory = new std::map<ttstr, std::pair<iTVPRenderManager *(*)(), iTVPRenderManager *>>;
...
public:
    __tTVPSoftwareRenderManagerAutoReigster() { TVPRegisterRenderManager("software", TVPGetSoftwareRenderManager); }
