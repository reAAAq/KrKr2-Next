...
//---------------------------------------------------------------------------
tjs_error TJS_INTF_METHOD tTJSNI_BaseVideoOverlay::Construct(tjs_int numparams, tTJSVariant **param,
    ...
    if(TJS_FAILED(
           clo.Object->NativeInstanceSupport(TJS_NIS_GETINSTANCE, tTJSNC_Window::ClassID, (iTJSNativeInstance **)&win)))
        TVPThrowExceptionMessage(TVPSpecifyWindow);
    if(!win)
        TVPThrowExceptionMessage(TVPSpecifyWindow);
    Window = win;
...
tTJSNC_VideoOverlay::tTJSNC_VideoOverlay() : tTJSNativeClass(TJS_W("VideoOverlay")) {
    ...
    //----------------------------------------------------------------------
    TJS_BEGIN_NATIVE_PROP_DECL(position){ // Start: Add:	T.Imoto
