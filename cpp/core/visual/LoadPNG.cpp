...
//---------------------------------------------------------------------------
// user_malloc_fn
static png_voidp PNG_malloc(png_structp ps, png_size_t size) { return malloc(size); }
//---------------------------------------------------------------------------
// user_free_fn
static void PNG_free(png_structp ps, void * /* png_structp*/ mem) { free(mem); }
//---------------------------------------------------------------------------
// user_error_fn
static void PNG_error(png_structp ps, png_const_charp msg) { TVPThrowExceptionMessage(TVPPNGLoadError, msg); }
//---------------------------------------------------------------------------
// user_warning_fn
...
#define PNG_read_be32(a)                                                                                               \
    (((tjs_uint32)(a)[0] << 24) + ((tjs_uint32)(a)[1] << 16) + ((tjs_uint32)(a)[2] << 8) + ((tjs_uint32)(a)[3]))
...
void TVPLoadPNG(void *formatdata, void *callbackdata, tTVPGraphicSizeCallback sizecallback,
    ...
    try {
        // create png_struct
        png_ptr = png_create_read_struct_2(PNG_LIBPNG_VER_STRING, (png_voidp) nullptr, (png_error_ptr)PNG_error,
                                           (png_error_ptr)PNG_warning, (png_voidp) nullptr, (png_malloc_ptr)PNG_malloc,
        ...
        png_set_read_user_chunk_fn(png_ptr, reinterpret_cast<void *>(&read_chunk_callback_user_struct),
        ...
        if(metainfopushcallback && png_get_oFFs(png_ptr, info_ptr, &offset_x, &offset_y, &offset_unit_type)) {
        ...
        int reso_unit_type;
        if(metainfopushcallback && png_get_pHYs(png_ptr, info_ptr, &reso_x, &reso_y, &reso_unit_type)) {
            // push offset information into metainfo data
        ...
        if(mode == glmPalettized) {
            ...
            if(bit_depth > 8)
                TVPThrowExceptionMessage(TVPPNGLoadError, (const tjs_char *)TVPUnsupportedColorTypePalette);
            ...
            switch(color_type) {
                ...
                case PNG_COLOR_TYPE_RGB:
                    ...
                    break;
                default:
                    TVPThrowExceptionMessage(TVPPNGLoadError, (const tjs_char *)TVPUnsupportedColorType);
            }
        }
        ...
        if(png_get_interlace_type(png_ptr, info_ptr) == PNG_INTERLACE_NONE) {
            ...
            for(i = 0; i < height; i++) {
                ...
                if(!do_convert_rgb_gray) {
                    ...
                    TVPBLConvert24BitTo8Bit((tjs_uint8 *)scanline, (tjs_uint8 *)row_pointers[i], width);
                }
                scanlinecallback(callbackdata, -1);
            }
        }
 ...
 */
void TVPSaveAsPNG(void *formatdata, tTJSBinaryStream *dst, const iTVPBaseBitmap *image, const ttstr &mode,
                  iTJSDispatch2 *meta) {
    int bpp = 32;
    ...
    try {
        ...
        png_set_IHDR(png_ptr, info_ptr, width, height, 8, (bpp == 32 ? PNG_COLOR_TYPE_RGB_ALPHA : PNG_COLOR_TYPE_RGB),
