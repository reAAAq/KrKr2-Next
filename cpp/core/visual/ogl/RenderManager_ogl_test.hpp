        ...
        for(int x = 0; x < 256; ++x) {
            for(int y = 0; y < 256; ++y) {
                struct clr {
                    ...
                    unsigned char g;
                    unsigned char b;
                } *clr1 = (clr *)(testdata1 + 256 * y + x), *clr2 = (clr *)(testdata2 + 256 * y + x);
                clr1->a = 255 - x;
                clr2->a = 255 - y;
                clr1->r = x;
                clr2->r = y;
...
static void
_TEST_SHADER(const char *_method, iTVPRenderManager *_this,
    ...
    _this->OperateRect(method, texdest, nullptr, rc,
                       src_tex.empty() ? tRenderTexRectArray() : tRenderTexRectArray(&src_tex[0], src_tex.size()));
    if(ignoreAlpha)
        _TEST_IGNORE_ALPHA();
