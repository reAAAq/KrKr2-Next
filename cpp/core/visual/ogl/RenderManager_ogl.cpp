...
#define CHECK_GL_ERROR_DEBUG_WITH_FMT(fmt, ...)                                                                        \
    do {                                                                                                               \
        GLenum __error = glGetError();                                                                                 \
        if(__error) {                                                                                                  \
            ShowInMessageBox("OpenGL error 0x%04X in %s %s %d\n" fmt, __error, __FILE__, __FUNCTION__, __LINE__,       \
                             __VA_ARGS__);                                                                             \
        }                                                                                                              \
    } while(false)
#endif
...
#define GLAPIENTRY
    ...
    typedef void(GLAPIENTRY fCopyImageSubData)(GLuint srcName, GLenum srcTarget, GLint srcLevel, GLint srcX, GLint srcY,
    ...
    typedef void(GLAPIENTRY fClearTexSubImage)(uint texture, int level, int xoffset, int yoffset, int zoffset,
                                               GLsizei width, GLsizei height, GLsizei depth, GLenum format, GLenum type,
                                               const void *data);
    static fClearTexSubImage *glClearTexSubImage;

#ifdef _MSC_VER
    typedef void(GLAPIENTRY fGetTextureImage)(GLuint texture, GLint level, GLenum format, GLenum type, GLsizei bufSize,
                                              void *pixels);
...
bool TVPIsSupportTextureFormat(GLenum fmt) { return TVPTextureFormats.end() != TVPTextureFormats.find(fmt); }
...
static void TVPInitGLExtensionFunc() {
    ...
    GL_CHECK_shader_framebuffer_fetch = TVPCheckGLExtension("GL_EXT_shader_framebuffer_fetch") ||
        TVPCheckGLExtension("GL_ARM_shader_framebuffer_fetch") || TVPCheckGLExtension("GL_NV_shader_framebuffer_fetch");
...
static tjs_uint8 *TVPShrinkXYBy2_8(tjs_uint *dpitch, const tjs_uint8 *src, tjs_int spitch, tjs_uint srcw,
                                   tjs_uint srch) {
    tjs_uint dstw = (srcw + 1) / 2, dsth = (srch + 1) / 2;
...
static tjs_uint8 *TVPShrinkXBy2_8(tjs_uint *dpitch, const tjs_uint8 *src, tjs_int spitch, tjs_uint srcw,
                                  tjs_uint srch) {
    tjs_uint dstw = (srcw + 1) / 2;
    tjs_uint dp = *dpitch = (dstw + 7) & ~7;
    tjs_uint8 *ret = new tjs_uint8[dp * srch], *dst = ret;
...
static tjs_uint8 *TVPShrinkYBy2_8(tjs_uint *dpitch, const tjs_uint8 *src, tjs_int spitch, tjs_uint srcw,
                                  tjs_uint srch) {
    tjs_uint dsth = (srch + 1) / 2;
...
static tjs_uint8 *TVPShrink(tjs_uint *dpitch, const tjs_uint8 *src, tjs_int spitch, tjs_uint srcw, tjs_uint srch,
...
static tjs_uint8 *TVPShrink_8(tjs_uint *dpitch, const tjs_uint8 *src, tjs_int spitch, tjs_uint srcw, tjs_uint srch,
                              tjs_uint dstw, tjs_uint dsth) {
    if(srcw == dstw) {
        if((srch + 1) / 2 == dsth) {
...
static tjs_uint8 *TVPShrinkImage(TVPTextureFormat::e fmt, tjs_uint &dpitch, const tjs_uint8 *src, tjs_int spitch,
                                 tjs_uint srcw, tjs_uint srch, tjs_uint dstw, tjs_uint dsth) {
...
protected:
    ...
    public:
        ...
        AdapterTexture2D(iTVPTexture2D *owner, GLuint name, int w, int h) {
            ...
            _hasMipmaps = false;
            setGLProgram(
                cocos2d::GLProgramCache::getInstance()->getGLProgram(cocos2d::GLProgram::SHADER_NAME_POSITION_TEXTURE));
        }
...
public:
    tTVPOGLTexture2D_split(tTVPBitmap *bmp) :
        tTVPOGLTexture2D(bmp->GetWidth(), bmp->GetHeight(), _GetTextureFormatFromBPP(bmp->GetBPP()),
    ...
    virtual const void *GetScanLineForRead(tjs_uint l) { return Bitmap->GetScanLine(l); }
    ...
    void UpdateTextureData(GLTextureInfo &texinfo, const tTVPRect &rc) {
        ...
        if(texinfo.Width > GetMaxTextureWidth()) {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, internalfmt, texinfo.Width, texinfo.Height, 0, pixfmt, GL_UNSIGNED_BYTE,
                         nullptr);
    ...
    void AsSingleTexture() {
        ...
        cv::Mat src_img(Bitmap->GetHeight(), Bitmap->GetWidth(), CV_8UC4, (void *)Bitmap->GetBits(),
                        Bitmap->GetPitch());
...
public:
    // for manual init
    tTVPOGLTexture2D_static(TVPTextureFormat::e format, unsigned int tw, unsigned int th, float sw, float sh,
    ...
    tTVPOGLTexture2D_static(const void *data, int len, GLenum format, unsigned int tw, unsigned int th, unsigned int iw,
    ...
    void InitCompressedPixel(const void *data, unsigned int len, GLenum format, unsigned int width,
                             unsigned int height) {
        TVPCheckMemory();
        _glBindTexture2D(texture);
    ...
    void InitPixel(const void *pixel, unsigned int pitch, GLenum format, GLenum pixfmt, unsigned int intw,
                   unsigned int inth) {
        TVPCheckMemory();
        _glBindTexture2D(texture);
...
public:
    tTVPOGLTexture2D_mutatble(const void *pixel, int pitch, unsigned int w, unsigned int h, TVPTextureFormat::e format,
                              float sw, float sh) :
...
public:
    ...
    virtual tTVPOGLRenderMethod *SetBlendFuncSeparate(int func, int srcRGB, int dstRGB, int srcAlpha,
    ...
    bool (*CustomProc)(tTVPOGLRenderMethod *_method, tTVPOGLTexture2D *_tar, tTVPOGLTexture2D *reftar,
                       const tTVPRect *rctar, const tRenderTexRectArray &textures) = nullptr;
};
...
public:
    ...
    virtual void SetParameterColor4B(int id, unsigned int clr) {
        ...
        glUniform4f(id, (clr & 0xFF) / 255.0f, ((clr >> 8) & 0xFF) / 255.0f, ((clr >> 16) & 0xFF) / 255.0f,
                    (clr >> 24) / 255.0f);
    };
    ...
    virtual void Rebuild() {
        try {
            program = CombineProgram(GetVertShader(m_nTex), CompileShader(GL_FRAGMENT_SHADER, m_strScript));
        } catch(eTJSError &e) {
            e.AppendMessage("\n");
...
public:
    ...
    static bool DoFill(tTVPOGLRenderMethod *method, tTVPOGLTexture2D *tar, tTVPOGLTexture2D *, const tTVPRect *rctar,
        ...
        if(check) {
            ...
            GL::glGetTextureImage(tar->texture, 0, GL_BGRA, GL_UNSIGNED_BYTE,
                                  tar->GetInternalHeight() * tar->GetInternalWidth() * 4, _src.ptr(0, 0));
        }
#endif
        return true;
#if 0
...
class tTVPOGLRenderMethod_AdjustGamma : public tTVPOGLRenderMethod_Script {
    ...
    virtual void SetParameterPtr(int id, const void *v) {
        if(id == 0x3a33aad6) {
            ...
            glUniform3f(id_amp, (data.RCeil - data.RFloor) / 255.0f, (data.GCeil - data.GFloor) / 255.0f,
...
bool tTVPOGLTexture2D::RestoreNormalSize() {
    ...
    if(w < GetMaxTextureWidth() && h < GetMaxTextureHeight()) {
        ...
        TVPShowSimpleMessageBox(ttstr(TJS_W("Too large texture is not accessable by GPU.")),
...
static iTVPTexture2D *(*_CreateMutableTexture2D)(const void *pixel, int pitch, unsigned int w, unsigned int h,
                                                 TVPTextureFormat::e format);
...
protected:
    ...
    const char *const common_shader_framebuffer_fetch_prefix = "#if defined(GL_ARM_shader_framebuffer_fetch)"
                                                               "\n"
    ...
    const std::string ScriptCommonBltPrefix = ScriptCommonPrefix + "    vec4 d = texture2D(tex1, v_texCoord1);\n";
    const std::string ScriptCommonFBFBltPrefix = ScriptCommonPrefix + "    vec4 d = gl_LastFragColor;\n";
    ...
    T *CompileAndRegRegularBlendMethod(const char *name, const std::string &prefix, const std::string &code) {
        if(GL_CHECK_shader_framebuffer_fetch) {
            return CompileAndRegScript<T>(name, prefix + ScriptCommonFBFBltPrefix + code, 1,
        ...
        if(glGenFramebuffers == nullptr) {
            ...
            if(strstr(gl_extensions, "ARB_framebuffer_object")) {
                ...
                glGetRenderbufferParameteriv =
                    (PFNGLGETRENDERBUFFERPARAMETERIVPROC)wglGetProcAddress("glGetRenderbufferParameteriv");
                glIsFramebuffer = (PFNGLISFRAMEBUFFERPROC)wglGetProcAddress("glIsFramebuffer");
                glBindFramebuffer = (PFNGLBINDFRAMEBUFFERPROC)wglGetProcAddress("glBindFramebuffer");
                glDeleteFramebuffers = (PFNGLDELETEFRAMEBUFFERSPROC)wglGetProcAddress("glDeleteFramebuffers");
                glGenFramebuffers = (PFNGLGENFRAMEBUFFERSPROC)wglGetProcAddress("glGenFramebuffers");
                glCheckFramebufferStatus =
                    (PFNGLCHECKFRAMEBUFFERSTATUSPROC)wglGetProcAddress("glCheckFramebufferStatus");
                glFramebufferTexture1D = (PFNGLFRAMEBUFFERTEXTURE1DPROC)wglGetProcAddress("glFramebufferTexture1D");
                glFramebufferTexture2D = (PFNGLFRAMEBUFFERTEXTURE2DPROC)wglGetProcAddress("glFramebufferTexture2D");
                glFramebufferTexture3D = (PFNGLFRAMEBUFFERTEXTURE3DPROC)wglGetProcAddress("glFramebufferTexture3D");
                glFramebufferRenderbuffer =
                    (PFNGLFRAMEBUFFERRENDERBUFFERPROC)wglGetProcAddress("glFramebufferRenderbuffer");
                glGetFramebufferAttachmentParameteriv = (PFNGLGETFRAMEBUFFERATTACHMENTPARAMETERIVPROC)wglGetProcAddress(
                ...
                glGetRenderbufferParameteriv =
                    (PFNGLGETRENDERBUFFERPARAMETERIVPROC)wglGetProcAddress("glGetRenderbufferParameterivEXT");
                glIsFramebuffer = (PFNGLISFRAMEBUFFERPROC)wglGetProcAddress("glIsFramebufferEXT");
                glBindFramebuffer = (PFNGLBINDFRAMEBUFFERPROC)wglGetProcAddress("glBindFramebufferEXT");
                glDeleteFramebuffers = (PFNGLDELETEFRAMEBUFFERSPROC)wglGetProcAddress("glDeleteFramebuffersEXT");
                glGenFramebuffers = (PFNGLGENFRAMEBUFFERSPROC)wglGetProcAddress("glGenFramebuffersEXT");
                glCheckFramebufferStatus =
                    (PFNGLCHECKFRAMEBUFFERSTATUSPROC)wglGetProcAddress("glCheckFramebufferStatusEXT");
                glFramebufferTexture1D = (PFNGLFRAMEBUFFERTEXTURE1DPROC)wglGetProcAddress("glFramebufferTexture1DEXT");
                glFramebufferTexture2D = (PFNGLFRAMEBUFFERTEXTURE2DPROC)wglGetProcAddress("glFramebufferTexture2DEXT");
                glFramebufferTexture3D = (PFNGLFRAMEBUFFERTEXTURE3DPROC)wglGetProcAddress("glFramebufferTexture3DEXT");
                glFramebufferRenderbuffer =
                    (PFNGLFRAMEBUFFERRENDERBUFFERPROC)wglGetProcAddress("glFramebufferRenderbufferEXT");
                glGetFramebufferAttachmentParameteriv = (PFNGLGETFRAMEBUFFERATTACHMENTPARAMETERIVPROC)wglGetProcAddress(
    ...
    static iTVPTexture2D *CreateStaticTexture2D_solid(const void *dib, tjs_uint w, tjs_uint h, tjs_int pitch,
        ...
        if(TVPCheckSolidPixel((const tjs_uint8 *)dib, pitch, w, h)) {
            tTVPOGLTexture2D_static *ret =
                new tTVPOGLTexture2D_static(dib, 4, 1, 1, fmt, w, h, 1.0f / w, 1.0f / h, GL_NEAREST);
            return ret;
        }
        return nullptr;
    }
    ...
    static iTVPTexture2D *CreateStaticTexture2D_PVRTC(const void *dib, tjs_uint w, tjs_uint h, tjs_int pitch,
        ...
        ret = new tTVPOGLTexture2D_static(
            outBuf, pvrSize, isOpaque ? GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG : GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG, w, h,
            texsize, texsize, 1, 1);
    ...
    static iTVPTexture2D *CreateMutableTexture2D(const void *pixel, int pitch, unsigned int w, unsigned int h, float sw,
                                                 float sh, TVPTextureFormat::e fmt) {
        return new tTVPOGLTexture2D_mutatble(pixel, pitch, w, h, fmt, sw, sh);
    }

    static iTVPTexture2D *CreateMutableTexture2D_normal(const void *pixel, int pitch, unsigned int w, unsigned int h,
                                                        TVPTextureFormat::e fmt) {
        float sw = 1.f, sh = 1.f;
    ...
    static iTVPTexture2D *CreateMutableTexture2D_half(const void *pixel, int pitch, unsigned int w, unsigned int h,
                                                      TVPTextureFormat::e fmt) {
        float sw = 1.f, sh = 1.f;
        if(!pixel) {
            if(w > GetMaxTextureWidth()) {
    ...
    void InitGL() {
        ...
        std::string compTexMethod =
            IndividualConfigManager::GetInstance()->GetValue<std::string>("ogl_compress_tex", "none");
...
#define SATATUE_CASE(x)                                                                                                \
    case x:                                                                                                            \
        ...
        cocos2d::EventListenerCustom *listener =
            cocos2d::EventListenerCustom::create(EVENT_RENDERER_RECREATED, [this](cocos2d::EventCustom *) {
    ...
    TVPRenderManager_OpenGL() : tempTexture(nullptr), _drawCount(0) {
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>("ApplyColorMap",
                                                        opacityPrefix + colorPrefix + ScriptCommonPrefix +
                                                            ...
                                                            "}",
                                                        1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ZERO, GL_ONE);
        ...
        TEST_SHADER(RemoveOpacity, TVPRemoveOpacity_o(testdest, testrule, 256 * 256, TEST_SHADER_OPA),
                    src_tex[0].first = texdata[2]);
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script_BlendColor>("FillMask",
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script_BlendColor>("ConstColorAlphaBlend_a",
                                                                   colorPrefix +
                                                                       ...
                                                                       "}",
                                                                   0)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_CONSTANT_COLOR, GL_ONE_MINUS_CONSTANT_COLOR, GL_CONSTANT_COLOR,
                                   GL_ONE_MINUS_CONSTANT_COLOR);
        TEST_SHADER(ConstColorAlphaBlend_a,
                    TVPConstColorAlphaBlend_a(testdest, 256 * 256, TEST_SHADER_COLOR, TEST_SHADER_OPA));

        const char *shader_ConstColorAlphaBlend_d = "    d.a = opacity + d.a - opacity * d.a;\n"
        ...
        TEST_SHADER(ConstColorAlphaBlend_d, memcpy(testdest, testred, 256 * 256 * 4),
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>("RemoveConstOpacity",
                                                        opacityPrefix +
                                                            "void main(){\n"
                                                        ...
                                                        0)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ZERO, GL_ONE, GL_ZERO, GL_ONE_MINUS_SRC_ALPHA);
        TEST_SHADER(RemoveConstOpacity, TVPRemoveConstOpacity(testdest, 256 * 256, TEST_SHADER_OPA));
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script_BlendColor>("ConstAlphaBlend", copyShader, 1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_CONSTANT_COLOR, GL_ONE_MINUS_CONSTANT_COLOR, GL_ZERO, GL_ONE);
        TEST_SHADER(ConstAlphaBlend, TVPConstAlphaBlend_HDA(testdest, testdata1, 256 * 256, TEST_SHADER_OPA));
        ...
        TEST_SHADER(ConstAlphaBlend_a, TVPConstAlphaBlend_a(testdest, testdata1, 256 * 256, TEST_SHADER_OPA));
        ...
        CompileAndRegRegularBlendMethod("ConstAlphaBlend_d", opacityPrefix,
                                        ...
                                        "    gl_FragColor = d;\n"
                                        "}");
        TEST_SHADER(ConstAlphaBlend_d, TVPConstAlphaBlend_d(testdest, testdata1, 256 * 256, TEST_SHADER_OPA));
        ...
        std::string shader_AdjustGamma_a_1 = "uniform vec3 u_gamma;\n" // rgb
                                             ...
                                             "uniform vec3 u_amp;\n" // rgb
                                             "void main() {\n";
        std::string shader_AdjustGamma_a_2 = "    vec3 t = (s.rgb / (s.a + 0.001) - 1.0) * step(s.rgb, s.aaa + "
                                             "0.001) + 1.0;\n"
        ...
        if(GL_CHECK_shader_framebuffer_fetch) {
            CompileAndRegScript<tTVPOGLRenderMethod_AdjustGamma>(
                "AdjustGamma_a", shader_AdjustGamma_a_1 + "    vec4 s = gl_LastFragColor;\n" + shader_AdjustGamma_a_2,
                0, common_shader_framebuffer_fetch_prefix);
        } else {
            CompileAndRegScript<tTVPOGLRenderMethod_AdjustGamma>(
                "AdjustGamma_a",
                shader_AdjustGamma_a_1 + "    vec4 s = texture2D(tex0, v_texCoord0);\n" + shader_AdjustGamma_a_2, 1)
                ->SetTargetAsSrc();
        }
        ...
        method = CompileAndRegScript<tTVPOGLRenderMethod_Script>("ConstAlphaBlend_SD",
        ...
        CompileRenderMethod("ConstAlphaBlend_SD_d",
                            (opacityPrefix + ScriptCommonBltPrefix +
        ...
        method = CompileAndRegScript<tTVPOGLRenderMethod_UnivTrans>(
            ...
            "}",
            3);
        TEST_SHADER_IGNORE_ALPHA(UnivTransBlend, TVPInitUnivTransBlendTable(testtable, 64, 128),
        ...
        TEST_SHADER_IGNORE_ALPHA(UnivTransBlend, TVPInitUnivTransBlendTable(testtable, 192, 128),
                                 TVPUnivTransBlend(testdest, testdata1, testdata2, testrule, testtable, 256 * 256),
        ...
        method = CompileAndRegScript<tTVPOGLRenderMethod_UnivTrans>("UnivTransBlend_d",
                                                                    ...
                                                                    "}",
                                                                    3);
        TEST_SHADER(UnivTransBlend_d, TVPInitUnivTransBlendTable(testtable, 215, 128),
        ...
        TEST_SHADER(UnivTransBlend_d,
                    (TVPInitUnivTransBlendTable(testtable, 215 + 600, 600),
                     TVPUnivTransBlend_switch_d(testdest, testdata1, testdata2, testrule, testtable, 256 * 256,
                                                215 + 600, 215)),
                    method->SetParameterInt(method->EnumParameterID("vague"), 600),
        ...
        // --------- AlphaBlend ----------
#define TEST_SHADER_BLTO(x) TEST_SHADER(x, TVP##x##_HDA_o(testdest, testdata1, 256 * 256, TEST_SHADER_OPA));
        ...
        if(GL::glAlphaFunc) {
            ...
            CompileAndRegScript<tTVPOGLRenderMethod_Script>("AlphaBlend_color_AlphaTest",
                                                            ...
                                                            1)
                ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ZERO, GL_ONE);
            CompileAndRegScript<tTVPOGLRenderMethod_Script>("AlphaTest",
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script_BlendColor>("AlphaBlend_SD",
                                                                   /*opacityPrefix +*/ ScriptCommonPrefix +
                                                                       "    gl_FragColor = s;\n"
                                                                       "}",
                                                                   1)
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Perspective>("PerspectiveAlphaBlend_a",
                                                             opacityPrefix +
                                                                 "uniform mat3 M;\n"
                                                                 "void main() {\n"
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>("AlphaBlend_a",
                                                        ...
                                                        1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
        TEST_SHADER(AlphaBlend_a, TVPAlphaBlend_ao(testdest, testdata1, 256 * 256, TEST_SHADER_OPA));
        ...
        if(GL::glAlphaFunc) {
            ...
            CompileAndRegScript<tTVPOGLRenderMethod_Script>("AlphaBlend_color_a_AlphaTest",
                                                            colorPrefix + alpha_thresholdPrefix + ScriptCommonPrefix +
                                                                "    s *= color;\n"
                                                                "    if(s.a < alpha_threshold) discard;\n"
                                                                "    gl_FragColor = s;\n"
        ...
        TEST_SHADER(AlphaBlend_d, TVPAlphaBlend_do(testdest, testdata1, 256 * 256, TEST_SHADER_OPA));
        ...
        if(GL::glAlphaFunc) {
            CompileAndRegRegularBlendMethod<tTVPOGLRenderMethod_AlphaTest>("AlphaBlend_color_d_AlphaTest", colorPrefix,
                                                                           shader_AlphaBlend_d);
        } else {
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>("AdditiveAlphaBlend_a",
                                                        ...
                                                        1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ONE, GL_ONE_MINUS_SRC_ALPHA, GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
        TEST_SHADER(AdditiveAlphaBlend_a, TVPAdditiveAlphaBlend_ao(testdest, testdata1, 256 * 256, TEST_SHADER_OPA));

        // --------- ColorDodgeBlend ----------
        CompileAndRegScript<tTVPOGLRenderMethod_Script>("ColorDodgeBlend",
                                                        opacityPrefix + ScriptCommonBltPrefix +
        ...
        CompileAndRegRegularBlendMethod("LightenBlend", opacityPrefix,
                                        ...
                                        "    gl_FragColor = d;\n"
                                        "}");
        TEST_SHADER(LightenBlend, TVPLightenBlend_HDA_o(testdest, testdata1, 256 * 256, TEST_SHADER_OPA));
        ...
        const char *shader_PsSubBlend = "    d.rgb = mix(d.rgb, clamp(d.rgb + "
        ...
        const char *shader_PsMulBlend = "    s.a *= opacity;\n"
                                        ...
                                        "    gl_FragColor = s;\n"
                                        "}";
        CompileAndRegScript<tTVPOGLRenderMethod_Script>("PsMulBlend",
        ...
        shader_PsMulBlend = "    s *= color;\n"
                            ...
                            "    gl_FragColor = s;\n"
                            "}";
        CompileAndRegScript<tTVPOGLRenderMethod_Script>("PsMulBlend_color",
                                                        colorPrefix + ScriptCommonPrefix + shader_PsMulBlend, 1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ZERO, GL_SRC_COLOR, GL_ZERO, GL_ONE);
        if(GL::glAlphaFunc) {
            CompileAndRegScript<tTVPOGLRenderMethod_AlphaTest>("PsMulBlend_color_AlphaTest",
                                                               colorPrefix + ScriptCommonPrefix + shader_PsMulBlend, 1)
                ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ZERO, GL_SRC_COLOR, GL_ZERO, GL_ONE);
        } else {
            CompileAndRegScript<tTVPOGLRenderMethod_Script>("PsMulBlend_color_AlphaTest",
                                                            colorPrefix + alpha_thresholdPrefix + ScriptCommonPrefix +
                                                                "    s *= color;\n"
                                                                "    if(s.a < alpha_threshold) discard;\n"
                                                                "    s.rgb *= s.a;\n"
        ...
        const char *shader_PsScreenBlend = "    s.rgb *= opacity * s.a;\n"
                                           "    gl_FragColor = s;\n"
                                           "}";
        CompileAndRegScript<tTVPOGLRenderMethod_Script>("PsScreenBlend",
        ...
        shader_PsScreenBlend = "    s *= color;\n"
                               ...
                               "    gl_FragColor = s;\n"
                               "}";
        CompileAndRegScript<tTVPOGLRenderMethod_Script>("PsScreenBlend_color",
                                                        colorPrefix + ScriptCommonPrefix + shader_PsScreenBlend, 1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ONE_MINUS_DST_COLOR, GL_ONE, GL_ZERO, GL_ONE);
        if(GL::glAlphaFunc) {
            CompileAndRegScript<tTVPOGLRenderMethod_AlphaTest>(
                "PsScreenBlend_color_AlphaTest", colorPrefix + ScriptCommonPrefix + shader_PsScreenBlend, 1)
                ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ONE_MINUS_DST_COLOR, GL_ONE, GL_ZERO, GL_ONE);
        } else {
            CompileAndRegScript<tTVPOGLRenderMethod_Script>("PsScreenBlend_color_AlphaTest",
                                                            colorPrefix + alpha_thresholdPrefix + ScriptCommonPrefix +
                                                                "    s *= color;\n"
                                                                "    if(s.a < alpha_threshold) discard;\n"
                                                                "    s.rgb *= s.a;\n"
        ...
        const char *shader_AdditiveAlphaToAlpha = "    gl_FragColor = vec4(s.rgb / s.a, s.a);"
                                                  "}";
        if(GL_CHECK_shader_framebuffer_fetch) {
            CompileAndRegScript<tTVPOGLRenderMethod_Script>("AdditiveAlphaToAlpha",
                                                            std::string() +
                                                                "void main() {\n"
        ...
        // DoGrayScale
        const char *shader_DoGrayScale = "    float c = s.r * 0.0743 + s.g * 0.7148 + s.b * 0.2109;\n"
                                         "    s.r = c; s.g = c; s.b = c;\n"
                                         "    gl_FragColor = s;\n"
                                         "}";
        if(GL_CHECK_shader_framebuffer_fetch) {
            CompileAndRegScript<tTVPOGLRenderMethod_Script>("DoGrayScale",
                                                            std::string() +
                                                                "void main() {\n"
                                                                "    vec4 s = gl_LastFragColor;\n" +
                                                                shader_DoGrayScale,
            ...
            CompileAndRegScript<tTVPOGLRenderMethod_Script>("DoGrayScale", ScriptCommonPrefix + shader_DoGrayScale, 1)
                ->SetTargetAsSrc();
        }
        ...
        if(format > TVPTextureFormat::Compressed) {
            ...
            switch(format) {
                ...
                case TVPTextureFormat::Compressed + GL_COMPRESSED_RGBA8_ETC2_EAC:
                    ...
                    break;
                default:
                    TVPThrowExceptionMessage(TJS_W("Unsupported compressed texture format [%1]"),
                                             (tjs_int)(format - TVPTextureFormat::Compressed));
                    break;
            }
            if(w <= GetMaxTextureWidth() && h <= GetMaxTextureHeight()) {
                return new tTVPOGLTexture2D_static(pixel, blkw * blkh * blksize, format - TVPTextureFormat::Compressed,
                                                   w, h, w, h, 1, 1);
            }
    ...
    virtual iTVPTexture2D *CreateTexture2D(unsigned int neww, unsigned int newh, iTVPTexture2D *tex) override {
        tTVPOGLTexture2D *newtex =
            static_cast<tTVPOGLTexture2D *>(_CreateMutableTexture2D(nullptr, 0, neww, newh, tex->GetFormat()));
    ...
    virtual iTVPTexture2D *CreateTexture2D(tTJSBinaryStream *src) {
        ...
        if(fmt) {
            ...
            if(src->Read(&buf.front(), pixsize) != pixsize)
                return nullptr;
            if(fmt == GL_BGRA_EXT && !TVPCheckGLExtension("GL_EXT_texture_format_BGRA8888") &&
    ...
    void CopyTexture(tTVPOGLTexture2D *dst, tTVPOGLTexture2D *src, const tTVPRect &rcsrc) {
        if(GL::glCopyImageSubData && !src->IsCompressed && src->_scaleW == dst->_scaleW &&
           src->_scaleH == dst->_scaleH && src->Format == dst->Format) {
            ...
            if(rc.get_width() == 0 || rc.get_height() == 0)
                return;
            GL::glCopyImageSubData(src->texture, GL_TEXTURE_2D, 0, rc.left, rc.top, 0, dst->texture, GL_TEXTURE_2D, 0,
                                   0, 0, 0, rc.get_width(), rc.get_height(), 1);
            CHECK_GL_ERROR_DEBUG();
#ifdef _DEBUG
            static bool check = false;
            if(check) {
                ...
                GL::glGetTextureImage(src->texture, 0, GL_BGRA, GL_UNSIGNED_BYTE, src->internalH * src->internalW * 4,
                                      _src.ptr(0, 0));
                GL::glGetTextureImage(dst->texture, 0, GL_BGRA, GL_UNSIGNED_BYTE, dst->internalH * dst->internalW * 4,
                                      _dst.ptr(0, 0));
                dst = dst;
            }
#endif
    ...
    virtual void OperateRect(iTVPRenderMethod *_method, iTVPTexture2D *_tar, iTVPTexture2D *reftar,
        ...
        method->Apply();
        tar->AsTarget();
        glViewport(rctar.left * tar->_scaleW, rctar.top * tar->_scaleH, rctar.get_width() * tar->_scaleW,
        ...
        if(check) {
            ...
            for(unsigned int i = 0; i < texlist.size(); ++i) {
                ...
                GL::glGetTextureImage(texlist[i].tex->texture, 0, GL_BGRA, GL_UNSIGNED_BYTE,
                                      texlist[i].tex->internalH * texlist[i].tex->internalW * 4, _src[i]->ptr(0, 0));
            }
            cv::Mat _tar(tar->internalH, tar->internalW, CV_8UC4);
            cv::Mat _stencil(tar->internalH, tar->internalW, CV_8U);
            GL::glGetTextureImage(tar->texture, 0, GL_BGRA, GL_UNSIGNED_BYTE, tar->internalH * tar->internalW * 4,
                                  _tar.ptr(0, 0));
            if(glIsEnabled(GL_STENCIL_TEST)) {
                glReadPixels(0, 0, tar->internalW, tar->internalH, GL_STENCIL_INDEX, GL_UNSIGNED_BYTE,
                             _stencil.ptr(0, 0));
            }
            tar = tar;
            for(unsigned int i = 0; i < texlist.size(); ++i) {
    ...
    virtual void OperateTriangles(iTVPRenderMethod *_method, int nTriangles, iTVPTexture2D *_tar, iTVPTexture2D *reftar,
    ...
    virtual void OperatePerspective(iTVPRenderMethod *_method, int nQuads, iTVPTexture2D *_tar, iTVPTexture2D *reftar,
                                    const tTVPRect &rcclip, const tTVPPointD *_pttar /*quad{lt,rt,lb,rb}*/,
                                    const tRenderTexQuadArray &textures) {
        ++_drawCount;
        tTVPOGLTexture2D *tar = (tTVPOGLTexture2D *)_tar;
        ...
        for(int i = 0; i < nQuads; ++i) {
            cv::Point2f pts_src[] = { cv::Point2f(srcpt[0].x / tw, srcpt[0].y / th),
                                      cv::Point2f((srcpt[1].x + 1) / tw, srcpt[1].y / th),
                                      cv::Point2f((srcpt[3].x + 1) / tw, (srcpt[3].y + 1) / th),
                                      cv::Point2f(srcpt[2].x / tw, (srcpt[2].y + 1) / th) };
            cv::Point2f pts_dst[] = { cv::Point2f(dstpt[0].x * tar->_scaleW, dstpt[0].y * tar->_scaleH),
