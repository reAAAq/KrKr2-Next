...
[[maybe_unused]] void cocos_android_app_init(JNIEnv *env) { // for cocos3.10+
    ...
    static auto core_logger = spdlog::android_logger_mt("core", "KrKr2NativeCore");
    static auto tjs2_logger = spdlog::android_logger_mt("tjs2", "KrKr2NativeTjs2");
    static auto plugin_logger = spdlog::android_logger_mt("plugin", "KrKr2NativePlugin");
    ...
    if (handle) {
        ...
        if (!sdl2Init || ((JNI_OnLoad)sdl2Init)(vm, nullptr) != JNI_VERSION_1_4) {
...
void Java_org_tvp_kirikiri2_KR2Activity_initDump(JNIEnv *env, jclass cls, jstring path) {
    ...
    if(pszPath && *pszPath) {
        ...
        static google_breakpad::ExceptionHandler eh(descriptor, DumpFilter, DumpCallback, nullptr, true, -1);
    }
...
JNIEXPORT void JNICALL Java_org_tvp_kirikiri2_KR2Activity_nativeTouchesBegin(JNIEnv *env, jclass thiz, jint id,
                                                                             jfloat x, jfloat y) {
    intptr_t idlong = id;
    Android_PushEvents([idlong, x, y]() {
        cocos2d::Director::getInstance()->getOpenGLView()->handleTouchesBegin(1, (intptr_t *)&idlong, (float *)&x,
                                                                              (float *)&y);
    });
}

JNIEXPORT void JNICALL Java_org_tvp_kirikiri2_KR2Activity_nativeTouchesEnd(JNIEnv *env, jclass thiz, jint id, jfloat x,
                                                                           jfloat y) {
    intptr_t idlong = id;
    Android_PushEvents([idlong, x, y]() {
        cocos2d::Director::getInstance()->getOpenGLView()->handleTouchesEnd(1, (intptr_t *)&idlong, (float *)&x,
                                                                            (float *)&y);
    });
}

JNIEXPORT void JNICALL Java_org_tvp_kirikiri2_KR2Activity_nativeTouchesMove(JNIEnv *env, jclass thiz, jintArray ids,
    ...
    if(size == 1) {
        ...
        idlong = id;
        Android_PushEvents([idlong, x, y]() {
            cocos2d::Director::getInstance()->getOpenGLView()->handleTouchesMove(1, (intptr_t *)&idlong, (float *)&x,
                                                                                 (float *)&y);
        });
        return;
    }
    ...
    Android_PushEvents([idlong, x, y]() {
        cocos2d::Director::getInstance()->getOpenGLView()->handleTouchesMove(idlong.
...
JNIEXPORT void JNICALL Java_org_tvp_kirikiri2_KR2Activity_nativeTouchesCancel(JNIEnv *env, jclass thiz, jintArray ids,
                                                                              jfloatArray xs, jfloatArray ys) {
    int size = env->GetArrayLength(ids);
    if(size == 1) {
        intptr_t idlong;
        ...
        idlong = id;
        Android_PushEvents([idlong, x, y]() {
            cocos2d::Director::getInstance()->getOpenGLView()->handleTouchesCancel(1, (intptr_t *)&idlong, (float *)&x,
                                                                                   (float *)&y);
        });
        return;
    }
    ...
    Android_PushEvents([idlong, x, y]() {
        cocos2d::Director::getInstance()->getOpenGLView()->handleTouchesCancel(idlong.
...
JNIEXPORT jboolean JNICALL Java_org_tvp_kirikiri2_KR2Activity_nativeKeyAction(JNIEnv *env, jclass cls, jint keyCode,
                                                                              jboolean isPress) {
...
JNIEXPORT void JNICALL Java_org_tvp_kirikiri2_KR2Activity_nativeInsertText(JNIEnv *env, jclass cls, jstring text) {
    ...
    if(pszText && *pszText) {
        ...
        Android_PushEvents([str]() {
            cocos2d::IMEDispatcher::sharedDispatcher()->dispatchInsertText(str.
...
JNIEXPORT void JNICALL Java_org_tvp_kirikiri2_KR2Activity_nativeCharInput(JNIEnv *env, jclass cls, jint keyCode) {
    ...
    if(!pScene)
        return;
    pScene->getScheduler()->performFunctionInCocosThread([keyCode] { TVPMainScene::onCharInput(keyCode); });
}

JNIEXPORT void JNICALL Java_org_tvp_kirikiri2_KR2Activity_nativeCommitText(JNIEnv *env, jclass cls, jstring text,
    ...
    pScene->getScheduler()->performFunctionInCocosThread([str] { TVPMainScene::onTextInput(str); });
...
JNIEXPORT void JNICALL Java_org_tvp_kirikiri2_KR2Activity_nativeHoverMoved(JNIEnv *env, jclass cls, jfloat x,
                                                                           jfloat y) {
