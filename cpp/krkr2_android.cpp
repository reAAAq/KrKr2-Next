...
//	__InitAndroidDump();
    ...
    sinks.push_back(
            std::make_shared<spdlog::sinks::android_sink_mt>("core")
    );
    sinks.push_back(
            std::make_shared<spdlog::sinks::android_sink_mt>("plugin")
    );

    // 创建多目标日志器
    auto logger = std::make_shared<spdlog::logger>(
            "KrKr2Native", sinks.begin(), sinks.end()
    );
...
Java_org_tvp_kirikiri2_KR2Activity_nativeTouchesMove(
        JNIEnv
*env,
jclass thiz, jintArray
ids,
jfloatArray xs, jfloatArray
ys
) {
...
jfloat y;
env->
GetIntArrayRegion(ids,
0, size, &id);
env->
GetFloatArrayRegion(xs,
0, size, &x);
env->
GetFloatArrayRegion(ys,
0, size, &y);
idlong = id;
Android_PushEvents([idlong, x, y]() {
cocos2d::Director::getInstance()
...
JNIEXPORT void JNICALL
Java_org_tvp_kirikiri2_KR2Activity_nativeInsertText(JNIEnv
*env,
jclass cls, jstring
text) {
const char *pszText = env->GetStringUTFChars(text, nullptr);
...
env->
ReleaseStringUTFChars(text, pszText
);
}

JNIEXPORT void JNICALL
Java_org_tvp_kirikiri2_KR2Activity_nativeDeleteBackward(JNIEnv
*env,
jclass cls
) {
Android_PushEvents(
[
capture0 = cocos2d::IMEDispatcher::sharedDispatcher()
] {
capture0->
...
JNIEXPORT void JNICALL
Java_org_tvp_kirikiri2_KR2Activity_nativeCharInput(JNIEnv
*env,
jclass cls, jint
keyCode) {
...
[keyCode] {
TVPMainScene::onCharInput(keyCode);
});
}
...
pScene->getScheduler()->performFunctionInCocosThread(
[str] {
TVPMainScene::onTextInput(str);
});
env->
ReleaseStringUTFChars(text, utftext
);
}

JNIEXPORT jboolean
JNICALL
        Java_org_tvp_kirikiri2_KR2Activity_nativeGetHideSystemButton(JNIEnv * env, jclass
cls) {
...
JNIEXPORT void JNICALL
Java_org_tvp_kirikiri2_KR2Activity_nativeHoverMoved(JNIEnv
*env,
jclass cls, jfloat
x,
jfloat y
...
JNIEXPORT void JNICALL
Java_org_tvp_kirikiri2_KR2Activity_nativeMouseScrolled(JNIEnv
*env,
jclass cls, jfloat
v) {
...
JNIEXPORT void JNICALL
Java_org_tvp_kirikiri2_KR2Activity_nativeOnLowMemory(JNIEnv
*env,
jclass cls
) {
Android_PushEvents([]() {
