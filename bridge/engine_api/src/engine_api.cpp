...
namespace {
    ...
    std::shared_ptr<spdlog::sinks::sink> g_file_log_sink;
    std::string g_runtime_log_file_path;
    ...
    std::string ResolveRuntimeLogFilePath() {
        if(!g_runtime_log_file_path.empty()) {
        ...
        if(const char *home = std::getenv("HOME"); home != nullptr && home[0] != '\0') {
            log_dir = fs::path(home) / "Library" / "Logs" / "KrKr2Next";
        } else {
            log_dir = fs::temp_directory_path(ec) / "KrKr2Next";
            if(ec) {
                log_dir = fs::path("/tmp") / "KrKr2Next";
            }
        }

        fs::create_directories(log_dir, ec);
        g_runtime_log_file_path = (log_dir / "engine-runtime.log").string();
        return g_runtime_log_file_path;
    }

    void LogNativeBacktrace(const char *reason) {
#if !defined(__ANDROID__)
        void *frames[64];
        ...
        if(symbols) {
            for(int i = 0; i < count; ++i) {
                spdlog::critical("{} [{}] {}", reason, i, symbols[i]);
            }
            free(symbols);
        }
#else
    ...
    void TerminateHandler() {
        try {
            if(auto current = std::current_exception(); current != nullptr) {
                try {
                    std::rethrow_exception(current);
                } catch(const std::exception &e) {
                    spdlog::critical("std::terminate called with std::exception: {}", e.what());
                } catch(...) {
                    spdlog::critical("std::terminate called with non-std exception");
                }
            } else {
                spdlog::critical("std::terminate called without active exception");
            }
        } catch(...) {
            spdlog::critical("TerminateHandler failed while inspecting active exception");
        }

        LogNativeBacktrace("terminate");
    ...
    void EnsureRuntimeLoggersInitialized() {
        std::call_once(g_loggers_init_once, []() {
            ...
            try {
                g_file_log_sink = std::make_shared<spdlog::sinks::basic_file_sink_mt>(
                    ResolveRuntimeLogFilePath(), true);
            } catch(const std::exception &e) {
                spdlog::default_logger()->error("Failed to create runtime log file sink: {}", e.what());
            } catch(...) {
                spdlog::default_logger()->error("Failed to create runtime log file sink: unknown error");
            }
            auto attach_sink =
                [](const std::shared_ptr<spdlog::logger> &logger) {
                    ...
                    auto attach_one =
                        [&](const std::shared_ptr<spdlog::sinks::sink> &target) {
                            if(target == nullptr) {
                                return;
                            }
                            ...
                            const auto already_attached = std::any_of(
                                ...
                                [&](const std::shared_ptr<spdlog::sinks::sink> &sink) {
                                    return sink.get() == target.get();
                                });
                            if(!already_attached) {
                                sinks.push_back(target);
                            }
                        };
                    attach_one(g_startup_log_sink);
                    attach_one(g_file_log_sink);
                };
            ...
            spdlog::info("Runtime log file: {}", ResolveRuntimeLogFilePath());
...
engine_result_t engine_destroy(engine_handle_t handle) {
    ...
    if(owned_runtime) {
        spdlog::info("engine_destroy: begin runtime teardown");
        try {
            spdlog::info("engine_destroy: Application->OnDeactivate begin");
            Application->OnDeactivate();
            spdlog::info("engine_destroy: Application->OnDeactivate end");
        } catch(...) {
            spdlog::error("engine_destroy: Application->OnDeactivate threw");
        }
        spdlog::info("engine_destroy: clearing pending user messages");
        Application->FilterUserMessage(
            [](std::vector<std::tuple<void *, int, tTVPApplication::tMsg>>
                   &queue) { queue.clear(); });

        // -----------------------------------------------------------------------
        // Proper runtime teardown — allows subsequent engine_open_game to
        // succeed.
        // -----------------------------------------------------------------------

        // 1. Unregister internal plugins while the script engine is still alive,
        // then run Application::OnExit → TVPUninitScriptEngine + delete TVPSystemControl.
        try {
        ...
        // 2. TVPSystemUninit → TVPUninitTVPGL + TVPCauseAtExit (at-exit
        // handlers)
        //    Note: TVPUninitScriptEngine inside will be a no-op since it was
        //    already called by OnExit (guarded by TVPScriptEngineUninit flag).
        try {
        ...
        // 3. Destroy EngineLoop and MainScene singletons
        if(auto *scene = TVPMainScene::GetInstance()) {
            spdlog::info("engine_destroy: deleting TVPMainScene singleton");
            delete scene;
        }
        if(auto *loop = EngineLoop::GetInstance()) {
            spdlog::info("engine_destroy: deleting EngineLoop singleton");
            delete loop;
        }

        // 4. Reset all internal state for restart
        spdlog::info("engine_destroy: resetting restartable runtime state");
        ...
        if(g_engine_bootstrapped) {
            spdlog::info("engine_destroy: TVPEngineBootstrap::Shutdown begin");
            TVPEngineBootstrap::Shutdown();
            g_engine_bootstrapped = false;
            spdlog::info("engine_destroy: TVPEngineBootstrap::Shutdown end");
        }

        // 5. Clear the once-started flag so the next engine_open_game can
        // proceed
        g_runtime_started_once = false;
        spdlog::info("engine_destroy: runtime teardown complete");
        spdlog::default_logger()->flush();
    }
...
engine_result_t engine_set_option(engine_handle_t handle,
    ...
    if(key == ENGINE_OPTION_ANGLE_BACKEND) {
        ...
        spdlog::info("engine_set_option: angle_backend={}",
                        option->value_utf8);
        ClearHandleErrorLocked(impl);
        SetThreadError(nullptr);
        return ENGINE_RESULT_OK;
...
engine_result_t engine_set_render_target_iosurface(engine_handle_t handle,
    ...
    if(iosurface_id == 0) {
        ...
        spdlog::info(
            "engine_set_render_target_iosurface: attached id={} {}x{}",
...
engine_result_t engine_set_render_target_surface(engine_handle_t handle,
    ...
    if(native_window == nullptr) {
        ...
        impl->render.native_window_attached = true;
        spdlog::info("engine_set_render_target_surface: attached {}x{}",
                        width, height);
